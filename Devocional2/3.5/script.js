...
function abrirModalPrincipal() {
    ...
    if (estado.devocionalEditando) {
        preencherFormularioEdicao(estado.devocionalEditando);
    }
...
/**
 * Monta a lista de ministrantes: os fixos + todos os nomes
 * já usados em devocionais anteriores (cadastrados via "Outro")
 * @returns {Array<string>} Lista de nomes de ministrantes
 */
function obterListaMinistrantes() {
    const fixos = ['Maicon', 'Daniel Feix', 'Antônio'];

    const doHistorico = estado.historicoDevocionais
        .map(d => (d.ministrado_por || '').trim())
        .filter(nome => nome !== '' && !fixos.includes(nome));

    const customizados = [...new Set(doHistorico)].sort((a, b) => a.localeCompare(b, 'pt-BR'));

    return [...fixos, ...customizados];
}

/**
 * Renderiza os radio buttons de "Ministrado por", incluindo
 * nomes personalizados já usados em devocionais anteriores
 */
function renderizarOpcoesMinistrante() {
    const container = document.getElementById('radioOptionsGrid');
    if (!container) return;

    const nomes = obterListaMinistrantes();

    const opcoesHTML = nomes.map(nome => `
        <label class="radio-option">
            <input type="radio" name="ministradoPor" value="${escapeHTML(nome)}">
            <span class="radio-checkmark"></span>
            <span class="radio-label">${escapeHTML(nome)}</span>
        </label>
    `).join('');
    ...
    container.innerHTML = `
        ${opcoesHTML}
        <label class="radio-option">
            <input type="radio" name="ministradoPor" value="outro" id="radioOutro">
            <span class="radio-checkmark"></span>
            <span class="radio-label">Outro</span>
        </label>
        <label class="radio-option">
            <input type="radio" name="ministradoPor" value="" id="radioVazio">
            <span class="radio-checkmark"></span>
            <span class="radio-label">Nenhum</span>
        </label>
    `;
...
function preencherFormularioEdicao(devocional) {
    ...
    const ministradoPor = (devocional.ministrado_por || '').trim();
    let radioCorrespondente = null;

    document.querySelectorAll('input[name="ministradoPor"]').forEach(radio => {
        if (radio.id !== 'radioVazio' && radio.value === ministradoPor) {
            radioCorrespondente = radio;
        }
    });

    if (ministradoPor !== '' && radioCorrespondente) {
        radioCorrespondente.checked = true;
        elementos.inputMinistradoPor.value = ministradoPor;
        elementos.inputMinistradoPor.disabled = true;
    } else if (ministradoPor !== '') {
...
function atualizarPainelDevocional() {
    ...
    const anterior = ordenados.findLast(d => parseData(d.data) < inicioSemana);
    const proximo = ordenados.find(d => parseData(d.data) > fimSemana);
