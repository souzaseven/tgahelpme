...
class ControlePausa {
    ...
    public function getEstadoAtual() {
        try {
            $stmt = $this->pdo->query("SELECT * FROM controle_pausa ORDER BY nome");
    ...
    public function entrarNaEspera($nome) {
        try {
            ...
            // VERIFICAÇÃO: Se já está na fila, não pode entrar novamente
            ...
            // SEMPRE vai para a espera primeiro
            ...
            if ($result) {
                error_log("SUCESSO: {$nome} entrou na lista de espera");
    ...
    public function ficarSegundo($nome) {
        try {
            ...
            // Buscar todas as pessoas na espera ordenadas
            ...
            // VERIFICAÇÃO CRÍTICA: Só permite se for o PRIMEIRO da fila
            if ($posicaoAtual !== 0) {
                return ['success' => false, 'error' => 'nao_primeiro', 'message' => 'Apenas o primeiro da fila pode escolher ficar como segundo'];
            }

            // VERIFICAÇÃO: Precisa ter pelo menos uma pessoa atrás para trocar
            if (count($fila) < 2) {
                return ['success' => false, 'error' => 'fila_curta', 'message' => 'Não há pessoas suficientes na fila para esta operação'];
            }

            // LÓGICA CORRIGIDA: Trocar o primeiro com o segundo
            $segundo = $fila[1]; // A segunda pessoa da fila
            
            // Pega os timestamps atuais
            ...
            // Troca os timestamps - o segundo vai para primeiro (timestamp mais antigo)
            // e o primeiro vai para segundo (timestamp mais recente)
    ...
    public function moverParaUltimaPosicao($nome) {
        try {
            ...
            // Busca a última pessoa na fila
            $stmt = $this->pdo->prepare("SELECT nome FROM controle_pausa WHERE status = 'espera' ORDER BY inicio_espera DESC LIMIT 1");
            $stmt->execute();
            $ultimo = $stmt->fetch();
            
            if ($ultimo) {
                // Coloca a pessoa no final (timestamp mais recente que o último)
                $timestamp = date('Y-m-d H:i:s', strtotime('+1 second'));
                $stmt = $this->pdo->prepare("UPDATE controle_pausa SET inicio_espera = ? WHERE nome = ?");
                $result = $stmt->execute([$timestamp, $nome]);
                
                if ($result) {
                    error_log($nome . " movido para última posição");
                }
                
                return ['success' => $result];
            }
            return ['success' => false, 'error' => 'sem_fila'];
        } catch (Exception $e) {
    ...
    public function sairDaPausa($nome) {
        try {
            ...
            if ($atual && $atual['status'] == 'pausa') {
                ...
                // NÃO mover próximo automaticamente - ele precisa confirmar
