<?php
// =============================================================
// controle_pausa.php (v2.6) - Seguro + Fila Inteligente + Sessão Admin + Logout Forçado
// =============================================================
...
function validarAdminSessao(): bool {
    return isset($_SESSION['admin_autenticado'], $_SESSION['admin_user'])
        && $_SESSION['admin_autenticado'] === true
        && strtolower($_SESSION['admin_user']) === 'anderson';
}
...
// =============================================================
// 🔹 Verifica existência do operador
// =============================================================
...
try {
    switch ($acao) {
        ...
        // ---------------------------------------------------------
        // 🔹 ENTRAR EM PAUSA (somente 1º da fila e se houver vaga)
        // ---------------------------------------------------------
        case 'entrar_pausa':
            $nomeReal = resolverNomeCompleto($pdo, $nomeEntrada);
            if (!$nomeReal) throw new Exception("Operador não encontrado ($nomeEntrada).");
        ...
        // ---------------------------------------------------------
        // 🔹 DERRUBAR LOGIN (Admin)
        // ---------------------------------------------------------
        case 'derrubar_login':
            if (!validarAdminSessao())
                throw new Exception("Acesso negado.");

            session_unset();
            session_destroy();

            echo json_encode(['success' => true, 'msg' => 'Sessão encerrada pelo administrador.']);
            break;
