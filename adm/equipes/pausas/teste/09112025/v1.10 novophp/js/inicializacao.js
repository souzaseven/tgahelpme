// ============================================================
// inicializacao.js - Sistema de Pausas (v1.9)
// ------------------------------------------------------------
// 🔹 Responsável pela inicialização da interface e login
// 🔹 Aguarda controle_pausa.js via evento "controle:ready"
// 🔹 Inclui filtro de equipes persistente (Ver somente minha equipe)
// 🔹 Compatível com setup_sistema.php e main.js v1.7+
// ============================================================

console.log("%c[Init] 🚀 Inicializando interface do sistema de pausas (v1.9)...", "color:#00ff88;font-weight:bold;");
...
function inicializarSistema() {
  ...
  async function fazerLogin(nome, senha = "") {
    try {
      ...
      if (data.success) {
        ...
        localStorage.setItem("usuario", data.nome_canonico);
  ...
  if (btnTrocar) {
    btnTrocar.addEventListener("click", () => {
      if (confirm("Deseja realmente trocar de usuário?")) {
        ...
        document.querySelector(".usuario-logado")?.remove();
        setTimeout(() => {
  ...
  // ============================================================
  // 👥 Filtro de Equipes (Ver somente minha equipe)
  // ============================================================
  window.aplicarFiltroEquipes = function() {
    const filtroAtivo = localStorage.getItem("filtroEquipe") === "true";
    const usuario = (localStorage.getItem("operador_nome") || "").toLowerCase();
    const blocos = document.querySelectorAll(".equipe-bloco");

    blocos.forEach(bloco => {
      const nomes = bloco.querySelectorAll(".op-item strong");
      const contemUsuario = Array.from(nomes).some(el => el.textContent.toLowerCase() === usuario);
      bloco.style.display = filtroAtivo ? (contemUsuario ? "block" : "none") : "block";
    });

    // Atualiza visual do botão se existir
    const btn = document.getElementById("btnToggleEquipes");
    if (btn) {
      btn.textContent = filtroAtivo ? "Ver todas as equipes" : "Ver somente minha equipe";
      btn.classList.toggle("ativo", filtroAtivo);
    }
  };

  // Alternar ao clicar no botão
  document.addEventListener("click", (e) => {
    if (e.target.id === "btnToggleEquipes") {
      const ativo = localStorage.getItem("filtroEquipe") === "true";
      localStorage.setItem("filtroEquipe", !ativo);
  ...
  if (!operadorLogado) {
    ...
    const iniciarControle = () => {
      if (window.controle && typeof window.controle.iniciar === "function") {
        ...
        aplicarFiltroEquipes(); // 🔹 Aplica filtro automaticamente ao iniciar
      } else {
