...
class ControlePausaSistema {
  ...
  criarItemOperador(p) {
    ...
    if (admin) {
      botoes.innerHTML = `
        <button class="btn-acao" onclick="window.controle.enviarAcao('entrar_fila',{nome:'${p.nome}',equipe:'${p.equipe}'})">🕓 Espera</button>
        <button class="btn-acao" onclick="window.controle.enviarAcao('forcar_pausa',{nome:'${p.nome}',equipe:'${p.equipe}'})">☕ Pausa</button>
        <button class="btn-acao" onclick="window.controle.enviarAcao('voltar_disponivel',{nome:'${p.nome}',equipe:'${p.equipe}'})">✅ Disponível</button>
      `;
    } else if (ehUser) {
