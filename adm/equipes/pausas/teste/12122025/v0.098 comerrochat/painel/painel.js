...
<div class="linha-participante ${expirado ? "pausa-expirada" : ""} ${op.elider == 1 ? "lider" : ""}"
    ...
    ${
    baseSeg >= 1140 &&                 // 19 minutos
    op.whatsapp &&                     // número existe
    op.whatsapp_optin == 1 &&          // opt-in ativo
    op.whatsapp_limite_enviado != 1    // ainda não avisado
    ? `
        <div class="linha-fila-acoes">
           <button
    class="btn btn-warning btn-aviso-pausa"
    onclick="enviarAvisoPausaManual(${id}, this)">
    ⚠️ Avisar limite de pausa
</button>
...
// ============================================================
// ENVIO MANUAL — AVISO DE LIMITE DE PAUSA (WHATSAPP)
// ============================================================
window.enviarAvisoPausaManual = async function (id, btn) {
    ...
    try {
        ...
        if (data.success) {
            ...
            throw new Error(data.erro || "Falha ao enviar aviso");
        }

    } catch (e) {
        console.error("[AVISO PAUSA MANUAL]", e);
        ...
        alert("Erro ao enviar aviso de pausa.");
    }
};
