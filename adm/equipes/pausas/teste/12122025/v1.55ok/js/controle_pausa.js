...
class ControlePausaSistema {
  ...
  this.intervaloAtualizacao = 2000; // 🔁 Atualiza a cada 1 segundo
...
if (window.aplicarBotoesOperador) {
  ...
  botoesPorStatus(p) {
    ...
    switch (p.status) {
      case "ativo":
      case "disponivel":
        return `<button class="btn-acao" onclick="window.controle.enviarAcao('entrar_fila',{nome:'${nome}',equipe:'${eq}'})">🕓 Fila de espera</button>`;
      case "espera":
        return `
  ...
  try {
    ...
    if (ret.success) {
      ...
      // ✅ Atualiza estado local e reflete imediatamente
      await this.atualizarEstado();
      this.atualizarInterfaceStatus(dados.nome, acao);

      // 🔔 Notifica visualmente os colegas
      this.notificarEquipeStatus(dados.nome, acao);
    } else {
...
// ============================================================
// Notifica toda a equipe visualmente e via Windows Notification
// ============================================================
notificarEquipeStatus(nome, acao) {
  ...
  const mensagem =
    acao.includes("pausa")
      ? `${nome} entrou em pausa ☕`
      : acao.includes("fila")
      ? `${nome} entrou na fila de espera 🕓`
      : `${nome} voltou a ficar disponível ✅`;

  // Toast visual para todos da equipe
  if (this.operador && todos.some(p => this.normalizar(p.nome) === this.normalizar(this.operador))) {
    this.toast(mensagem);
  }

  // Notificação de sistema (Windows)
...
// ============================================================
// Atualiza visual e botões do operador conforme o status atual
// ============================================================
atualizarInterfaceStatus(nome, acao) {
  const nomeNorm = this.normalizar(nome);
  const operadorItem = [...document.querySelectorAll(".op-item")].find(
    op => this.normalizar(op.querySelector("strong")?.textContent) === nomeNorm
  );
  ...
  // Atualiza botão visível (para o operador logado)
  ...
  if (novoStatus === "pausa") {
    botoes.innerHTML = `
      <button class="btn-acao" onclick="controle.enviarAcao('voltar_disponivel',{nome:'${nome}',equipe:'${this.buscarEquipePorOperador(nome)}'})">✅ Sair da pausa</button>
    `;
  } else if (novoStatus === "espera") {
    botoes.innerHTML = `
      <button class="btn-acao" onclick="controle.enviarAcao('voltar_disponivel',{nome:'${nome}',equipe:'${this.buscarEquipePorOperador(nome)}'})">❌ Sair da fila</button>
    `;
  } else {
    botoes.innerHTML = `
      <button class="btn-acao" onclick="controle.enviarAcao('entrar_fila',{nome:'${nome}',equipe:'${this.buscarEquipePorOperador(nome)}'})">🕓 Fila de espera</button>
      <button class="btn-acao" onclick="controle.enviarAcao('forcar_pausa',{nome:'${nome}',equipe:'${this.buscarEquipePorOperador(nome)}'})">☕ Entrar em pausa</button>
    `;
  }
}

// ============================================================
// Localiza a equipe do operador ativo
// ============================================================
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // 🔔 Solicita permissão para notificações (somente se o navegador suportar)
  try {
    if ("Notification" in window && Notification.permission !== "granted") {
      Notification.requestPermission().then(result => {
        if (result === "granted") {
          console.log("%c[Notificações ativadas]", "color:#00ff88");
        } else {
    ...
    console.warn("⚠️ Notificações não suportadas neste navegador.", e);
  }
});
