...
log_debug("✅ Transação finalizada com sucesso ({$acao}) para {$nome}/{$equipe}");

log_debug("⚠️ Rollback executado em {$acao} para {$nome}/{$equipe}");
...
case 'forcar_todos_disponivel': {
  ...
  log_debug("⚙️ Requisição: forcar_todos_disponivel por {$admin}");
  ...
  try {
    ...
    // Atualize todos com status pausa/espera/expirada para disponivel
    $pdo->exec("UPDATE operadores SET status='disponivel', posicao_fila=NULL, tempo_entrada=NULL WHERE status IN ('pausa','espera','expirada')");
    echo json_encode(['success'=>true, 'msg'=>'Todos disponiveis']); 
    exit;
