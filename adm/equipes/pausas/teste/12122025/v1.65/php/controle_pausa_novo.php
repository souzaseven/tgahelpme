...
/**
 * Controle de Pausas v1.15 (backend)
...
// ======================================================
// 🔍 SISTEMA DE LOG SIMPLES (debug/logs/controle_YYYY-MM-DD.log)
// ======================================================
...
// Limpa logs antigos (opcional - mantém últimos 5 dias)
foreach (glob(__DIR__ . "/logs/controle_*.log") as $file) {
  if (filemtime($file) < time() - 432000) unlink($file);
}

// ↓↓↓ ADICIONE após normalizar(), antes da criação de tabelas
function contarNaFila(PDO $pdo, $equipe) {
  $st = $pdo->prepare("SELECT COUNT(*) FROM controle_pausa WHERE equipe = ? AND status = 'espera'");
  $st->execute([$equipe]);
  return (int)$st->fetchColumn();
}

// Opcional: reduzir impacto de locks longos
try {
...
try {
  switch ($acao) {
    ...
    case 'entrar_fila': {
      ...
      try {
        log_debug("📩 Iniciando entrar_fila para {$nome}/{$equipe}");
        ...
        // Se houver vaga, entra direto em pausa — apenas se solicitado manualmente
        // (ao clicar "Entrar em Pausa", não via botão "Entrar na Fila")
        if (($payload['acao_origem'] ?? '') === 'forcar_pausa' && contarPausasAtivas($pdo, $equipe) < MAX_PAUSAS_POR_EQUIPE) {
          ...
          $pdo->commit();
          log_debug("☕ $nome ($equipe) entrou em pausa diretamente (forçar pausa)");
        ...
        if ($pdo->inTransaction()) $pdo->rollBack();
        log_debug("💥 Rollback em entrar_fila ({$nome}/{$equipe}): ".$e->getMessage());
    ...
    case 'forcar_pausa': {
      ...
      try {
        log_debug("📩 Iniciando forcar_pausa para {$nome}/{$equipe}");
        $row = garantirUsuario($pdo, $nome, $equipe);

        // Se houver vaga E NÃO houver ninguém esperando, entra direto em pausa
        $haVaga = contarPausasAtivas($pdo, $equipe) < MAX_PAUSAS_POR_EQUIPE;
        $temGenteNaFila = contarNaFila($pdo, $equipe) > 0;

        if ($haVaga && !$temGenteNaFila) {
          $st = $pdo->prepare("UPDATE controle_pausa 
                              SET status='pausa', tempo_entrada=?, tempo_saida=NULL, posicao_fila=NULL, ultima_atualizacao=? 
                              WHERE id=?");
          $st->execute([agora(), agora(), $row['id']]);
          $pdo->commit();
          jexit(true, ['msg' => 'Entrou em pausa imediatamente.', 'status' => 'pausa']);
        ...
        if ($pdo->inTransaction()) $pdo->rollBack();
        log_debug("💥 Rollback em forcar_pausa ({$nome}/{$equipe}): ".$e->getMessage());
    ...
    case 'voltar_disponivel': {
      ...
      try {
        log_debug("📩 Iniciando voltar_disponivel para {$nome}/{$equipe}");
        $row = garantirUsuario($pdo, $nome, $equipe);
        ...
        if ($pdo->inTransaction()) $pdo->rollBack();
        log_debug("💥 Rollback em voltar_disponivel ({$nome}/{$equipe}): ".$e->getMessage());
    ...
    case 'forcar_todos_disponivel': {
      ...
      log_debug("📩 Iniciando forcar_todos_disponivel por {$admin}");

      if ($admin !== strtolower('Anderson de Souza')) {
        log_debug("🚫 Tentativa sem permissão: {$admin}");
        jexit(false, ['error' => 'Sem permissão para derrubar todos.']);
      }

      try {
        $totalAntes = $pdo->query("SELECT COUNT(*) FROM controle_pausa WHERE status IN ('pausa','espera','expirada')")->fetchColumn();
        $pdo->exec("UPDATE controle_pausa SET status='ativo', posicao_fila=NULL, tempo_entrada=NULL, tempo_saida=NULL, ultima_atualizacao=NOW() WHERE status IN ('pausa','espera','expirada')");
        $totalDepois = $pdo->query("SELECT COUNT(*) FROM controle_pausa WHERE status IN ('pausa','espera','expirada')")->fetchColumn();
        $afetados = $totalAntes - $totalDepois;

        log_debug("☄ Admin derrubou {$afetados} operadores → ativos");
        jexit(true, [
    ...
    case 'decidir_troca': {
      ...
      try {
        log_debug("📩 Iniciando decidir_troca para {$equipe}");
        ...
        if ($pdo->inTransaction()) $pdo->rollBack();
        log_debug("💥 Rollback em decidir_troca ({$equipe}): ".$e->getMessage());
