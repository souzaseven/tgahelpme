<?php
/**
 * Controle de Pausas v1.17 (backend)
 *
 ...
 * - Novo status: "aguardando" (vaga aberta aguardando confirmação do operador)
 ...
 * - Ações adicionais: forcar_todos_disponivel, solicitar_troca, decidir_troca, expirar_pausas
 *
...
// Topo da fila – apenas quem está em ESPERA (não inclui "aguardando")
...
try {
  switch ($acao) {
    ...
    case 'get_estado': {
      ...
      // tempo_espera_dinamico apenas para quem está em espera/aguardando
      foreach ($lista as &$r) {
    ...
    case 'entrar_fila': {
      ...
      try {
        ...
        // Se origem for "forcar_pausa" e houver vaga → entra direto em pausa
        if (
    ...
    // ---------------------------------------------------------
    // FORÇAR TODOS PARA DISPONÍVEL (ADMIN)
    // POST JSON: { admin }
    // ---------------------------------------------------------
    ...
    case 'solicitar_troca': {
      ...
      if (count($top2) < 2) {
        jexit(false, ['error' => 'Não há 2 pessoas na fila.']);
      }
      ...
      jexit(true, [
        'msg'     => 'Solicitação enviada ao primeiro da fila.',
        'primeiro'=> $primeiro,
        'segundo' => $segundo
      ]);
    }
