...
// ---------------------------------------------------------
    ...
    // ---------------------------------------------------------
    // NOVO: SOLICITAR TROCA (posições válidas)
    // ---------------------------------------------------------
    case 'solicitar_troca': {
      ...
      if ($equipe === '' || $solicitante === '' || $alvo === '') {
        jexit(false, ['error' => 'Dados insuficientes.']);
      }

      // Carregar fila completa com prepare/execute (CORREÇÃO DE SEGURANÇA)
      $stFila = $pdo->prepare("
      ...
      if (!$fila) jexit(false, ['error' => 'Fila vazia.']);

      // encontra solicitante
      $idxSolic = -1;
      foreach ($fila as $i => $f) {
      ...
      // Se não foi enviado alvo, retorna lista de alvos possíveis
      if ($alvo === '') {
        $alvos = [];

        if ($idxSolic === 1) {
          // segundo -> só troca com terceiro
          if (isset($fila[2])) $alvos[] = $fila[2]['nome_usuario'];
        } else if ($idxSolic > 1 && $idxSolic < count($fila)-1) {
          // posições intermediárias
          $alvos[] = $fila[$idxSolic-1]['nome_usuario'];
          $alvos[] = $fila[$idxSolic+1]['nome_usuario'];
        } else {
        ...
        jexit(true, [
          'msg'   => 'Selecione com quem deseja trocar.',
          'alvos' => $alvos
        ]);
      }

      // Se alvo foi enviado, criar solicitação
      $valido = false;
      if ($idxSolic === 1) {
        // segundo -> só troca com terceiro
        if (isset($fila[2]) && strtolower($fila[2]['nome_usuario']) === strtolower($alvo)) {
          $valido = true;
        }
      } else if ($idxSolic > 1 && $idxSolic < count($fila)-1) {
        // posições intermediárias
        if (strtolower($fila[$idxSolic-1]['nome_usuario']) === strtolower($alvo) ||
            strtolower($fila[$idxSolic+1]['nome_usuario']) === strtolower($alvo)) {
          $valido = true;
        }
      } else {
        // último -> só troca com penúltimo
      ...
      if (!$valido) {
        jexit(false, ['error' => 'Alvo inválido para troca.']);
      }

      // Criar solicitação na nova tabela
      $st = $pdo->prepare("
      ...
      jexit(true, ['msg' => 'Solicitação de troca enviada.']);
    }
