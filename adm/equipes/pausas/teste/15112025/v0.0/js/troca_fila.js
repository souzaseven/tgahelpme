// ============================================================
// troca_fila.js (v2.1)
// Regras oficiais de troca de posição da fila
// Compatível com interface_botoes.js v5.1 e controle_pausa.js v5.3
// ============================================================

console.log("%c[troca_fila.js] módulo de trocas (v2.1) carregado", "color:#00cfff;font-weight:bold;");
...
class TrocaFilaSistema {
    constructor() {
        ...
        // Processa trocas pendentes sempre que o estado é atualizado
        document.addEventListener("estado:atualizado", (ev) => {
            const dados = ev.detail || {};
            const trocas = dados.trocas || [];

            console.log("[troca_fila] evento estado:atualizado recebido → trocas:", trocas);

            if (Array.isArray(trocas) && trocas.length) {
                this.processarTrocasPendentes(trocas);
            }
        });
    }
    ...
    abrirSolicitacaoTroca(nomeSolic, equipe) {
        ...
        // 🔹 Primeiro é sempre alvo possível (quando não é o próprio)
        // =========================================
        ...
        if (pos === 2) {
            ...
            // intermediários: anterior e próximo
            if (fila[idx - 1]) listaAlvos.push(fila[idx - 1]);
            if (fila[idx + 1]) listaAlvos.push(fila[idx + 1]);
        }

        // PRIMEIRO É SEMPRE ALVO OPCIONAL (para todos exceto ele mesmo)
        const primeiro = fila[0];
    ...
    processarTrocasPendentes(lista) {
        ...
        console.log("[troca_fila] processando trocas para:", meuNome, lista);
