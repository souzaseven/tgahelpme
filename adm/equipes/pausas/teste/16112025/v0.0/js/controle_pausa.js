...
// - Compatível com interface_botoes.js (v3.0) e troca_fila.js
...
class ControlePausaSistema {
  ...
  // -----------------------------------------
  // HELPER: Saber se é ADMIN (CORRETO)
  // -----------------------------------------
  isAdmin() {
    const n = this.normalizar(this.operador || "");
    ...
    if (ADM_LIST.includes(n)) return true;

    // fallback: flag salva no login
    const modoAdmin = localStorage.getItem("modo_admin");
    return modoAdmin === "1" || modoAdmin === "true";
  }
  ...
  constructor() {
    ...
    this.intervaloAtualizacao = 2000; // 2s
    this.maxPausas = 2;
    ...
    this.mapaEstadoAnterior = new Map();
    ...
    // Operador logado (salvo no login.js)
...
async sincronizarAtualizacoes() {
  ...
  try {
    ...
    const novasTrocas = dados.trocas || [];
    this.trocas = novasTrocas;

    // ==========================================================
    // Sempre processa trocas (mesmo se estado não mudar)
    // ==========================================================
    document.dispatchEvent(
      new CustomEvent("estado:atualizado", { 
        detail: { 
          estado: novoEstado,
          trocas: novasTrocas 
        }
    ...
    const estadoIgual = JSON.stringify(novoEstado) === JSON.stringify(this.estado);

    // Se o estado não mudou, não precisa re-renderizar nada
    if (!estadoIgual) {
