<?php
/**
 * Controle de Pausas v1.20 (backend)
 *
...
try {
  switch ($acao) {
    ...
    case 'get_estado': {
      ...
      // NOVO: TROCAS (compatível com troca_fila.js v2.0)
      $stTrocas = $pdo->query("
        SELECT id, equipe, solicitante, alvo, status, mensagem, created_at, updated_at
        FROM controle_pausa_trocas
        WHERE status = 'pendente'
           OR created_at >= (NOW() - INTERVAL 5 MINUTE)
        ORDER BY created_at DESC
      ");
      $trocas = $stTrocas->fetchAll(PDO::FETCH_ASSOC);
    ...
    // Front envia: { id_troca, resposta: "aceitar"|"recusar" }
    // ---------------------------------------------------------
    case 'responder_troca': {
      ...
      // ACEITAR: trocar posições na fila
      ...
      try {
        ...
        if ($pdo->inTransaction()) $pdo->rollBack();
        log_debug("💥 Erro em responder_troca ({$id}): " . $e->getMessage());
        jexit(false, ['error' => 'Erro ao processar a troca.']);
      }
    }
