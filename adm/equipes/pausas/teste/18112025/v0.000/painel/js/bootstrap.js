...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // CORRETO → Define EQUIPE de forma válida
    window.EQUIPE = nomeEquipe;
    console.log("[DEBUG] EQUIPE setada globalmente:", window.EQUIPE);
    ...
    if (btnToggleSidebar) {
        btnToggleSidebar.addEventListener("click", () => {
            ...
            if (icone) {
                icone.classList.toggle("fa-angle-left");
                icone.classList.toggle("fa-angle-right");
            }
        });
    }
    ...
    if (hud) hud.textContent = `Equipe: ${nomeEquipe}`;

    const resumo = document.getElementById("resumoEquipes");
    if (resumo) resumo.textContent = `Operador logado: ${nomeOperador} (${nomeEquipe})`;

    // ------------------------------
    // 5) Carregar operadores
    // ------------------------------
    ...
    // ------------------------------
    // 7) Inicializar botões INDIVIDUAIS do operador
    // ------------------------------
    if (window.iniciarBotoesOperador) {
        iniciarBotoesOperador(dados);
    } else {
        console.warn("[PAINEL] iniciarBotoesOperador não está disponível.");
    }
...
function carregarParticipantesEquipe(nomeEquipe, nomeOperador) {
    ...
    fetch(url)
        ...
        .then(dados => {
            ...
            if (hud) hud.textContent = `Equipe: ${nomeEquipe} • ${operadores.length} operadores`;
            if (resumo) resumo.textContent = `Equipe: ${nomeEquipe} • ${operadores.length} operadores cadastrados`;
            ...
            operadores.forEach(op => {
                ...
                // Define quem é o operador atual pelo NOME
                ...
                linha.innerHTML = `
                    ...
                    <!-- Botões ficam EMBAIXO do nome -->
                    <div class="acoes-operador-inline"></div>
                `;
...
function inicializarPreferencias() {
    ...
    if (btnFecharPref)  btnFecharPref.addEventListener("click", fecharModalPreferencias);
    if (btnSalvarPrefer) btnSalvarPrefer.addEventListener("click", salvarPreferencias);
    if (btnTesteAudio)   btnTesteAudio.addEventListener("click", testarAudioPreferencias);
    if (btnTesteNotif)   btnTesteNotif.addEventListener("click", testarNotificacaoPreferencias);
}
