...
function renderPainel() {
  ...
  agentes.forEach(a => {
    ...
    if (
      filtros.busca &&
      !(`${a.name}${a.login}`.toLowerCase().includes(filtros.busca))
    ) return;
  ...
  Object.entries(filas).forEach(([fila, lista]) => {
    ...
    lista.forEach(a => {
      ...
      // 🔘 Botão (fora do template)
      let botaoHTML = "";

      if (status === "paused" && podeDespausar(a)) {
        botaoHTML = `<button class="btn-despausar">Despausar</button>`;
      } else {
        botaoHTML = `<button ${!podePausar(a) ? "disabled" : ""}>Pausar</button>`;
      }
      ...
      if (status === "paused" && podeDespausar(a)) {
        btn.onclick = () => despausarAgente(a.id);
      } else if (podePausar(a)) {
        btn.onclick = () => abrirModalPausa(a.id);
      }
...
setInterval(() => {
  document.querySelectorAll(".agente[data-status='paused']").forEach(card => {
    ...
    if (tempoEl) {
      tempoEl.textContent = `⏱ ${tempoEmPausa(agente)}`;
    }
  });
}, 1000);
