<?php
// ============================================================
// contas.php — Contas bancárias/carteiras. `saldo_atual` é o valor
// que as outras telas debitam/creditam a cada transação paga;
// `saldo_inicial` fica congelado como referência histórica. O
// agrupamento por "titular" existe pra famílias/casais que dividem
// o mesmo painel mas querem ver o saldo de cada um separado.
//
// GET  (sem ação)             — contas ativas + agrupamento e totais por titular
// GET  ?impacto=1&id=X        — quantas transações usam essa conta (antes de excluir)
// POST acao=salvar            — cria/edita a conta
// POST acao=ajustar_saldo     — correção manual de saldo (sem gerar transação)
...
if ($metodo === 'POST') {
    ...
    if ($acao === 'salvar') {
        ...
        $icone   = iconeOuPadrao($d['icone'] ?? null, 'university');
