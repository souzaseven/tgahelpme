<?php
// ============================================================
// recorrencias.php — "Contas fixas": modelo de despesa/receita que
// se repete (aluguel, assinatura, salário...). Este arquivo não guarda
// as transações em si, só o modelo; cada mês precisa "gerar" o
// lançamento real (uma transação pendente) a partir do modelo — ver
// gerarParaMes(), chamada automaticamente ao criar a recorrência (mês
// atual + próximo) e sob demanda pela ação 'gerar'.
//
// GET  ?mes=M&ano=AAAA        — lista recorrências e se já geraram lançamento no mês
// POST acao=salvar            — cria/edita o modelo
// POST acao=gerar             — gera os lançamentos pendentes de um mês
// POST acao=toggle            — ativa/desativa sem apagar o modelo
// DELETE ?id=X
// ============================================================
require_once __DIR__ . '/../../backend/banco/conexao.php';
require_once __DIR__ . '/../helpers/recorrencias_auto.php';

$p      = TABLE_PREFIX;
$metodo = $_SERVER['REQUEST_METHOD'];

// gerarParaMes() vem de backend/helpers/recorrencias_auto.php — também usada
// pela geração automática em todo carregamento de página (ver index.php).
