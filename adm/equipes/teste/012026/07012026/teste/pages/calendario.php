...
function renderGrade() {
    ...
    for (let d = 1; d <= diasNoMes; d++) {
        ...
        if (evts.length) {
            cell.onclick = () => abrirPanel(dateStr, d);
            cell.setAttribute('role', 'button');
            cell.tabIndex = 0;
            cell.setAttribute('aria-label', `Ver ${evts.length} lançamento${evts.length !== 1 ? 's' : ''} do dia ${d}`);
            cell.addEventListener('keydown', e => {
                if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); abrirPanel(dateStr, d); }
            });
        }
