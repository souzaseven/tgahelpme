...
$stmt = $pdo->prepare(
    "SELECT c.*,
     ...
     LEFT JOIN `{$p}transacoes` t
            ON t.cartao_id = c.id AND t.tipo = 'despesa' AND t.status != 'cancelado'
...
$categoriasReceita = $pdo->query(
    "SELECT id, nome FROM `{$p}categorias` WHERE ativo=1 AND tipo IN ('receita','ambos') ORDER BY nome"
)->fetchAll();
...
$tiposCartao = [
    'credito'          => ['label' => 'Cartão de Crédito', 'icon' => 'fa-credit-card',  'badge' => null],
    'vale_alimentacao' => ['label' => 'Vale Alimentação',  'icon' => 'fa-utensils',      'badge' => 'VA'],
    'vale_refeicao'    => ['label' => 'Vale Refeição',     'icon' => 'fa-bowl-food',     'badge' => 'VR'],
];
?>
...
<div class="kpi-grid" style="grid-template-columns:repeat(3,1fr);margin-bottom:1.25rem">
    <div class="kpi-card violet">
        ...
        <div class="kpi-value"><?= brl($totLimite) ?></div>
    ...
    <div class="kpi-card rose">
        ...
        <div class="kpi-value"><?= brl($totUsado) ?></div>
    ...
    <div class="kpi-card emerald">
        ...
        <div class="kpi-value"><?= brl($totDisp) ?></div>
        <div class="kpi-trend neutral">
...
<div class="card">
    <div class="card-body" style="text-align:center;padding:3rem;color:var(--text-600)">
        ...
        <div class="text-sm" style="margin-bottom:1.25rem">Adicione seus cartões de crédito ou vale-alimentação/refeição para acompanhar limites e faturas.</div>
...
<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(280px,1fr));gap:1rem" id="cartoesGrid">
    <?php foreach ($cartoes as $cc):
        ...
        $tipoCc = $cc['tipo'] ?: 'credito';
        $ehVale = $tipoCc !== 'credito';
        $infoTipo = $tiposCartao[$tipoCc] ?? $tiposCartao['credito'];
    ...
    <div class="cc-full"
         ...
         role="button"
         tabindex="0"
         aria-label="Ver detalhes do cartão <?= htmlspecialchars($cc['nome']) ?>"
         style="background:linear-gradient(135deg,<?= $cor ?>cc,<?= $cor ?>88)"
         onclick="selecionarCartao(<?= $cc['id'] ?>)"
         onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();selecionarCartao(<?= $cc['id'] ?>)}">
        ...
        <div style="display:flex;justify-content:space-between;align-items:flex-start">
            <div>
                <div class="cc-nome">
                    ...
                    <?php if ($ehVale): ?>
                    <span class="badge" style="background:rgba(255,255,255,.2);color:#fff;margin-left:.35rem;font-size:.62rem"><?= $infoTipo['badge'] ?></span>
                    <?php endif ?>
                </div>
                <div class="cc-info">
                    <?= $cc['banco'] ? htmlspecialchars($cc['banco']) . ' &nbsp;·&nbsp; ' : '' ?>
                    <?= $ehVale ? htmlspecialchars($infoTipo['label']) : ucfirst($cc['bandeira']) ?>
                </div>
            </div>
            <i class="<?= $ehVale ? 'fa-solid' : 'fa-brands' ?> <?= htmlspecialchars($ehVale ? $infoTipo['icon'] : ($bandeiras[$cc['bandeira']]['icon'] ?? 'fa-credit-card')) ?> cc-bandeira-icon" style="color:rgba(255,255,255,.7)"></i>
        </div>
        ...
        <div style="display:flex;justify-content:space-between;align-items:center">
            <div>
                <div class="cc-usado"><?= brl((float)$cc['limite_usado']) ?></div>
                <div class="cc-info">de <?= brl((float)$cc['limite_total']) ?> &nbsp;·&nbsp; <?= $uso ?>% usado</div>
            </div>
            <div style="text-align:right">
                <?php if ($ehVale): ?>
                <span class="fatura-badge paga"><i class="fa-solid fa-wallet fa-xs"></i> Pré-pago</span>
                <?php else: ?>
                ...
                <?php endif ?>
                <?php if ($gastos > 0): ?>
                <div class="cc-info" style="margin-top:.25rem"><?= brl($gastos) ?> este mês</div>
                <?php endif ?>
            </div>
        </div>
        ...
        <div class="cc-limit-row">
            <?php if ($ehVale): ?>
            <span>Cartão pré-pago &nbsp;·&nbsp; sem fatura</span>
            <?php else: ?>
            <span>Fecha dia <?= $cc['dia_fechamento'] ?> &nbsp;·&nbsp; Vence dia <?= $cc['dia_vencimento'] ?></span>
            <?php endif ?>
            <span><?= brl((float)$cc['limite_total'] - (float)$cc['limite_usado']) ?> disponível</span>
        </div>
        ...
        <div style="display:flex;justify-content:flex-end;gap:.4rem;margin-top:.5rem"
            ...
            <button class="btn btn-sm"
                    style="background:rgba(255,255,255,.2);color:#fff;font-size:.72rem;padding:.25rem .6rem"
                    onclick="abrirModalRecargaById(<?= $cc['id'] ?>)"
                    title="Adicionar crédito">
                <i class="fa-solid fa-plus fa-xs"></i> Recarregar
            </button>
            <?php endif ?>
            <button class="btn btn-sm"
...
<div id="detalheSection">
    <div class="card">
        <div class="card-header" style="flex-wrap:wrap;gap:.75rem">
            ...
            <div style="display:flex;align-items:center;gap:.75rem;flex-wrap:wrap">
                <div style="text-align:right">
                    <div class="text-xs text-muted" id="detTotalLabel">Total da fatura</div>
                ...
                <button class="btn btn-primary btn-sm" id="btnRecarregarDet" style="display:none" onclick="abrirModalRecarga()">
                    <i class="fa-solid fa-plus"></i> Recarregar
                </button>
        ...
        <form id="formCartao" onsubmit="salvarCartao(event)">
            <div class="modal-body">
                ...
                <div class="form-group" style="margin-bottom:1.1rem">
                    ...
                    <select id="cTipo" class="form-control" onchange="onTipoCartaoChange()">
                    ...
                    <div class="text-xs text-muted" style="margin-top:.35rem">
                        Vale-alimentação/refeição não tem fatura: o limite funciona como saldo, e você aumenta ele registrando uma recarga.
                    </div>
                </div>

                <div class="form-grid form-grid-2" style="margin-bottom:1.1rem">
                    <div class="form-group" id="grpBandeira">
...
<!-- ── Modal: Recarregar (vale-alimentação/refeição) ─────── -->
<div id="modalRecarga" class="modal-overlay" onclick="if(event.target===this)fecharModais()">
    <div class="modal-box sm">
        <div class="modal-header">
            <div class="modal-title">Adicionar Crédito</div>
            <button type="button" class="btn-icon" onclick="fecharModais()">
                <i class="fa-solid fa-xmark"></i>
            </button>
        </div>
        <form id="formRecarga" onsubmit="salvarRecarga(event)">
            <div class="modal-body">
                <div class="card" style="margin-bottom:1.1rem;background:var(--bg-700)">
                    <div class="card-body" style="padding:.875rem">
                        <div class="text-xs text-muted">Cartão</div>
                        <div class="fw-700" id="recCartaoInfo" style="font-size:1rem;margin-top:.2rem">—</div>
                    </div>
                </div>

                <div class="form-group" style="margin-bottom:1.1rem">
                    <label class="form-label">Valor da recarga (R$) <span style="color:var(--rose)">*</span></label>
                    <input type="text" id="rValor" class="form-control"
                           placeholder="0,00" inputmode="numeric" data-currency required>
                </div>
                <div class="form-group" style="margin-bottom:1.1rem">
                    <label class="form-label">Data <span style="color:var(--rose)">*</span></label>
                    <input type="date" id="rData" class="form-control" required>
                </div>
                <div class="form-group" style="margin-bottom:1.1rem">
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="rDescricao" class="form-control"
                           placeholder="Ex: Recarga mensal" maxlength="255">
                </div>
                <div class="text-xs text-muted" style="margin-top:.75rem">
                    Isso cria uma receita e aumenta o limite (saldo) do cartão automaticamente.
                </div>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-ghost" onclick="fecharModais()">Cancelar</button>
                <button type="submit" class="btn btn-primary" id="btnSalvarRec">
                    <i class="fa-solid fa-plus"></i> Adicionar Crédito
                </button>
            </div>
        </form>
    </div>
...
function renderDetalhe(data) {
    ...
    const ehVale = cartao.tipo && cartao.tipo !== 'credito';
    ...
    document.getElementById('detTotalLabel').textContent = ehVale ? 'Gasto no mês' : 'Total da fatura';

    if (ehVale) {
        const badge = document.getElementById('detFatStatus');
        badge.className = 'fatura-badge paga';
        badge.innerHTML = '<i class="fa-solid fa-wallet fa-xs"></i> Pré-pago';
        document.getElementById('btnPagarFatura').style.display   = 'none';
        document.getElementById('btnRecarregarDet').style.display = '';
    } else {
        ...
        document.getElementById('btnRecarregarDet').style.display = 'none';
    }
...
// ── Modal: Novo/Editar Cartão ─────────────────────────────
function onTipoCartaoChange() {
    const ehVale = document.getElementById('cTipo').value !== 'credito';
    document.getElementById('grpBandeira').style.display = ehVale ? 'none' : '';
    document.getElementById('grpFech').style.display     = ehVale ? 'none' : '';
    document.getElementById('grpVenc').style.display      = ehVale ? 'none' : '';
}

function abrirModalCartao() {
    document.getElementById('formCartao').reset();
    document.getElementById('cId').value   = '';
    document.getElementById('cTipo').value = 'credito';
...
function _preencherModalCartao(cc) {
    ...
    document.getElementById('cTipo').value     = cc.tipo     || 'credito';
...
// ── Modal: Recarregar (vale-alimentação/refeição) ─────────
let _recargaCartaoId = null;

function _abrirModalRecargaParaCartao(cc) {
    _recargaCartaoId = cc.id;
    document.getElementById('formRecarga').reset();
    document.getElementById('rData').value = new Date().toISOString().split('T')[0];
    document.getElementById('recCartaoInfo').textContent = cc.nome;
    document.getElementById('modalRecarga').classList.add('open');
    setTimeout(() => document.getElementById('rValor').focus(), 80);
}

function abrirModalRecarga() {
    if (!_detalhe) return;
    _abrirModalRecargaParaCartao(_detalhe.cartao);
}

function abrirModalRecargaById(id) {
    const cc = _CARTOES.find(c => +c.id === +id);
    if (cc) _abrirModalRecargaParaCartao(cc);
}

async function salvarRecarga(e) {
    e.preventDefault();
    if (!_recargaCartaoId) return;
    const btn = document.getElementById('btnSalvarRec');
    btn.disabled = true;
    btn.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> Adicionando...';

    const valor = parseCurrency(document.getElementById('rValor').value);
    const payload = {
        descricao:    document.getElementById('rDescricao').value.trim() || 'Recarga de cartão',
        valor:        valor,
        data:         document.getElementById('rData').value,
        categoria_id: document.getElementById('rCategoria').value || null,
        cartao_id:    _recargaCartaoId,
        status:       'pago',
    };
    ...
    try {
        ...
        const json = await res.json();
        if (!json.success) throw new Error(json.erro);
        toast(json.msg || 'Crédito adicionado!', 'success');
        fecharModais();
        location.reload();
    } catch (err) {
        toast('Erro: ' + err.message, 'error');
        ...
        btn.disabled = false;
        btn.innerHTML = '<i class="fa-solid fa-plus"></i> Adicionar Crédito';
    }
}
