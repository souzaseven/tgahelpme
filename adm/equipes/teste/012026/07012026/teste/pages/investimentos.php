...
<div class="kpi-grid" style="grid-template-columns:repeat(4,1fr);margin-bottom:1.25rem">
    <div class="kpi-card violet">
        ...
        <div class="kpi-value"><?= brl($totAplicado) ?></div>
    ...
    <div class="kpi-card <?= $rendimento >= 0 ? 'emerald' : 'rose' ?>">
        ...
        <div class="kpi-value"><?= ($rendimento >= 0 ? '+' : '') . brl($rendimento) ?></div>
        <div class="kpi-trend <?= $rendimento >= 0 ? 'up' : 'down' ?>">
            <i class="fa-solid fa-<?= $rendimento >= 0 ? 'arrow-up' : 'arrow-down' ?> fa-xs"></i>
            <?= ($rendimento >= 0 ? '+' : '') . pct($rentab, 2) ?> total
        </div>
    </div>
    <div class="kpi-card <?= $rentab >= 0 ? 'emerald' : 'rose' ?>">
        <div class="kpi-header">
            <div class="kpi-label">Rentabilidade</div>
            <div class="kpi-icon <?= $rentab >= 0 ? 'emerald' : 'rose' ?>"><i class="fa-solid fa-percent"></i></div>
        </div>
        <div class="kpi-value"><?= ($rentab >= 0 ? '+' : '') . pct($rentab, 2) ?></div>
            ...
            <div style="flex:1;min-width:150px;display:flex;flex-direction:column;gap:.4rem">
                ...
                <div class="d-flex align-center justify-between text-sm">
                    ...
                    <div class="d-flex align-center gap-1">
                        ...
                        <span class="fw-600"><?= brl($val) ?></span>
                    </div>
                </div>
                <?php endforeach ?>
            </div>
        ...
        <div class="card-body" style="display:flex;flex-direction:column;gap:.875rem">
            ...
            <div>
                <div class="d-flex justify-between text-sm" style="margin-bottom:.3rem">
                    ...
                    <span><?= brl($rendaFixaTot) ?> &nbsp;<span class="text-muted">(<?= $pctRF ?>%)</span></span>
                </div>
            ...
            <div>
                <div class="d-flex justify-between text-sm" style="margin-bottom:.3rem">
                    ...
                    <span><?= brl($rendaVarTot) ?> &nbsp;<span class="text-muted">(<?= $pctRV ?>%)</span></span>
                </div>
                <div class="progress-bar-bg">
            ...
            <div class="d-flex align-center justify-between text-sm">
                ...
                <div class="text-right">
                    <div class="fw-600"><?= brl((float)$inv['valor_atual']) ?></div>
        ...
        <table>
            ...
            <tr class="inv-row" data-id="<?= $inv['id'] ?>" onclick="selecionarInv(<?= $inv['id'] ?>)">
                ...
                <td class="text-sm">
                    ...
                    <span class="fw-600"><?= pct((float)$inv['rentabilidade_taxa'], 2) ?></span>
                    <span class="text-muted text-xs"> <?= $rentTipoLabels[$inv['rentabilidade_tipo']] ?? '' ?></span>
                    <?php else: ?>
                    <span class="text-muted text-xs"><?= $rentTipoLabels[$inv['rentabilidade_tipo']] ?? '—' ?></span>
                    <?php endif ?>
                </td>
                <td class="text-right text-sm text-muted"><?= brl((float)$inv['valor_aplicado']) ?></td>
                <td class="text-right fw-600"><?= brl((float)$inv['valor_atual']) ?></td>
                <td class="text-right <?= $rentCls ?>">
                    <?= ($rend >= 0 ? '+' : '') . brl($rend) ?><br>
                    <span class="text-xs"><?= ($rentRow >= 0 ? '+' : '') . pct($rentRow, 2) ?></span>
                </td>
