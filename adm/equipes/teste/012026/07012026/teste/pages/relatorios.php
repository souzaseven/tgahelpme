...
<div class="tabs">
    ...
    <button class="tab-btn" onclick="ativarTab('curva_abc')">
        <i class="fa-solid fa-chart-column fa-xs"></i> Curva ABC
    </button>
...
<!-- ═══════════════════════════════════════════════════════════
     TAB 1 — VISÃO GERAL
═══════════════════════════════════════════════════════════ -->
<?php include __DIR__ . '/relatorios/_tab_resumo.php'; ?>

<!-- ═══════════════════════════════════════════════════════════
     TAB 2 — FLUXO DE CAIXA
═══════════════════════════════════════════════════════════ -->
<?php include __DIR__ . '/relatorios/_tab_fluxo.php'; ?>

<!-- ═══════════════════════════════════════════════════════════
     TAB 3 — CATEGORIAS
═══════════════════════════════════════════════════════════ -->
<?php include __DIR__ . '/relatorios/_tab_categorias.php'; ?>

<!-- ═══════════════════════════════════════════════════════════
     TAB 3b — CURVA ABC / PARETO
═══════════════════════════════════════════════════════════ -->
<?php include __DIR__ . '/relatorios/_tab_curva_abc.php'; ?>

<!-- ═══════════════════════════════════════════════════════════
     TAB 4 — PATRIMÔNIO
═══════════════════════════════════════════════════════════ -->
<?php include __DIR__ . '/relatorios/_tab_patrimonio.php'; ?>

<!-- ═══════════════════════════════════════════════════════════
     TAB 5 — RESUMO ANUAL
═══════════════════════════════════════════════════════════ -->
<?php include __DIR__ . '/relatorios/_tab_anual.php'; ?>

<!-- ═══════════════════════════════════════════════════════════
     TAB 6 — PREVISÃO DO MÊS
═══════════════════════════════════════════════════════════ -->
<?php include __DIR__ . '/relatorios/_tab_previsao.php'; ?>

<!-- ═══════════════════════════════════════════════════════════
     TAB 7 — RANKING
═══════════════════════════════════════════════════════════ -->
<?php include __DIR__ . '/relatorios/_tab_ranking.php'; ?>

<!-- ═══════════════════════════════════════════════════════════
     TAB 8 — POR RESPONSÁVEL
═══════════════════════════════════════════════════════════ -->
<?php include __DIR__ . '/relatorios/_tab_por_resp.php'; ?>

<!-- ═══════════════════════════════════════════════════════════
     TAB IR — IMPOSTO DE RENDA
═══════════════════════════════════════════════════════════ -->
<?php include __DIR__ . '/relatorios/_tab_ir.php'; ?>
...
function ativarTab(id) {
    document.querySelectorAll('.tab-btn').forEach((b, i) => {
        const tabs = ['resumo','fluxo','categorias','curva_abc','patrimonio','anual','previsao','ranking','por_resp','ir'];
    ...
    if (!_tabLoaded[id]) {
        ...
        if (id === 'curva_abc')  carregarCurvaAbc();
...
function renderCategorias(data, mes, ano, tipo) {
    ...
    if (!cats.length) {
        ...
        document.getElementById('catTbody').innerHTML   = '<tr><td colspan="5" class="empty-state sm">Sem lançamentos neste período.</td></tr>';
        return;
    }
...
async function carregarCurvaAbc() {
    const mes  = document.getElementById('abcMes').value;
    const ano  = document.getElementById('abcAno').value;
    const tipo = document.getElementById('abcTipo').value;

    try {
        const res  = await fetch(`backend/api/relatorios.php?acao=curva_abc&mes=${mes}&ano=${ano}&tipo=${tipo}`);
        const json = await res.json();
        if (!json.success) throw new Error(json.erro);
        renderCurvaAbc(json, tipo);
    } catch (err) { toast('Erro: ' + err.message, 'error'); }
}

const CORES_CLASSE_ABC = { A: '#f43f5e', B: '#f59e0b', C: '#64748b' };

function renderCurvaAbc(data, tipo) {
    const cats = data.dados;
    const qtd  = data.qtd_por_classe || { A: 0, B: 0, C: 0 };

    document.getElementById('abcResumoA').textContent = qtd.A + ' categoria' + (qtd.A === 1 ? '' : 's');
    document.getElementById('abcResumoB').textContent = qtd.B + ' categoria' + (qtd.B === 1 ? '' : 's');
    document.getElementById('abcResumoC').textContent = qtd.C + ' categoria' + (qtd.C === 1 ? '' : 's');

    if (!cats.length) {
        document.getElementById('abcTbody').innerHTML =
            '<tr><td colspan="6" class="empty-state sm">Sem lançamentos neste período.</td></tr>';
        mkChart('chartAbc', 'bar', { labels: [], datasets: [] }, {});
        return;
    }

    mkChart('chartAbc', 'bar', {
        labels: cats.map(c => c.nome),
        datasets: [
            {
                type: 'bar',
                label: tipo === 'despesa' ? 'Despesa' : 'Receita',
                data: cats.map(c => c.total),
                backgroundColor: cats.map(c => CORES_CLASSE_ABC[c.classe]),
                borderRadius: 4,
                yAxisID: 'y',
            },
            {
        ...
        scales: {
            x: { grid: { color: gridColor }, ticks: { color: fontColor, font: { size: 10 } } },
            y: {
                position: 'left',
            ...
            y1: {
                ...
                min: 0, max: 100,
                grid: { display: false },
                ticks: { color: fontColor, font: { size: 11 }, callback: v => v + '%' },
            },
        },
    });

    document.getElementById('abcTbody').innerHTML = cats.map((c, i) =>
        `<tr>
            <td class="text-muted text-sm">#${i + 1}</td>
            <td>
                <div class="d-flex align-center gap-1">
            ...
            <td class="text-right fw-600 ${tipo === 'despesa' ? 'text-rose' : 'text-emerald'}">${brl(c.total)}</td>
            <td class="text-right text-sm">${c.pct}%</td>
            <td class="text-right text-sm fw-600">${c.pct_acum}%</td>
            <td class="text-right">
                <span class="badge" style="background:${CORES_CLASSE_ABC[c.classe]}22;color:${CORES_CLASSE_ABC[c.classe]}">
                    ${c.classe}
                </span>
            </td>
        </tr>`
...
// a versão local daqui não escapava aspas; a global escapa.
...
function renderRankingCats(cats) {
    ...
    if (!cats || !cats.length) {
        ...
        el.innerHTML = `<div class="empty-state" style="font-size:.85rem">
...
function renderPrevisaoTabela(itens, mes, ano) {
    ...
    if (!itens.length) {
        document.getElementById('pvTbody').innerHTML =
            `<tr><td colspan="6" class="empty-state lg">
