...
@media (max-width: 768px) {
    ...
    /* Comportamento mobile do modal (sobe da base) já vem de main.css. */
}
</style>
...
<div id="painelTerceiro" style="display:none">
    ...
    <div class="kpi-grid" style="margin-bottom:1.25rem" id="terceiroKpis">
        ...
        <div class="kpi-card emerald" id="tKpiSaldoCard">
            <div class="kpi-header"><div class="kpi-label">Saldo do Mês</div>
            <div class="kpi-icon emerald" id="tKpiSaldoIcon"><i class="fa-solid fa-scale-balanced"></i></div></div>
...
function renderTerceiroCards(lista) {
    ...
    el.innerHTML = lista.map(t => {
        ...
        const saldo   = parseFloat(t.total_receitas) - parseFloat(t.total_despesas);
        const sBg     = saldo >= 0 ? 'rgba(16,185,129,.08)' : 'rgba(239,68,68,.08)';
        const sClass  = saldo >= 0 ? 'text-emerald' : 'text-rose';
        const sTxt    = `${saldo >= 0 ? '+' : ''}${brl(saldo)}`;
        return `<div class="terc-card ${isAtivo ? 'active' : ''}" data-id="${+t.id}"
            role="button" tabindex="0" aria-label="Ver detalhes de ${esc(t.nome)}"
            onclick="selecionarTerceiro(${+t.id})"
            onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();selecionarTerceiro(${+t.id})}">
            ...
            <div style="background:${sBg};border-radius:var(--radius);padding:.45rem .75rem;margin-bottom:.6rem;display:flex;align-items:center;justify-content:space-between">
                <span class="text-xs text-muted">Saldo Atual</span>
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:.4rem;font-size:.78rem">
                <div>
                    <div class="text-muted">Previsão Rec.</div>
                    <div class="fw-600 text-emerald">${brl(parseFloat(t.previsao_receitas))}</div>
                </div>
                <div>
                    <div class="text-muted">Previsão Desp.</div>
                    <div class="fw-600 text-rose">${brl(parseFloat(t.previsao_despesas))}</div>
                </div>
            </div>
...
function renderTabelaTerceiro(rows, kpi) {
    ...
    // Saldo pode ficar negativo (você deve pro terceiro) — a cor do card
    // precisa refletir isso em vez de ficar sempre na cor neutra do indigo.
    const corSaldo = saldo >= 0 ? 'emerald' : 'rose';
    document.getElementById('tKpiSaldoCard').className = `kpi-card ${corSaldo}`;
    document.getElementById('tKpiSaldoIcon').className = `kpi-icon ${corSaldo}`;
...
<div id="bulkBarTc" class="bulk-bar" style="display:none">
    <i class="fa-solid fa-square-check" style="color:var(--indigo);font-size:1rem"></i>
    <span id="bulkCountTc" class="bulk-bar-count">0 selecionados</span>
    <div style="flex:1"></div>
    <button class="bulk-bar-btn" onclick="abrirModalBulkStatusTc()">
        <i class="fa-solid fa-tag"></i> Alterar status
    </button>
    <button class="bulk-bar-btn danger" onclick="excluirSelecionadosTc()">
        <i class="fa-solid fa-trash"></i> Excluir selecionados
    </button>
    <button class="bulk-bar-btn ghost" onclick="limparSelecaoTc()">
        <i class="fa-solid fa-xmark"></i> Cancelar
    </button>
</div>

<!-- ── Modal alterar status em lote (terceiros) ─────────── -->
<div id="modalBulkStatusTc" class="bulk-modal-overlay" style="display:none"
     onclick="if(event.target===this)fecharModalBulkStatusTc()">
