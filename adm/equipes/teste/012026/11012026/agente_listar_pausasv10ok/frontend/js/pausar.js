...
function carregarFiltros() {
  ...
  // 🔁 Sincronizar UI
  if (chkSomentePausa) chkSomentePausa.checked = filtros.somentePausa;
  if (chkOffline) chkOffline.checked = filtros.mostrarOffline;
  if (chkArquiv) chkArquiv.checked = filtros.mostrarArquivados;
  if (chkInativos) chkInativos.checked = filtros.mostrarInativos;
}
...
function renderPainel() {
  ...
  if (contador) {
    contador.innerHTML = `
      ...
      <br><small>${Object.entries(contMotivos).map(([k,v])=>`${k}: ${v}`).join(" | ")}</small>
    `;
  }
