...
try {
  ...
  if ($action === 'list') {
    ...
    if ($q !== '') {
      $where[] = "(cod_cliente LIKE ? OR cliente LIKE ? OR acesso_server LIKE ? OR observacao LIKE ?)";
      $like = "%$q%";
    ...
    $stmtT = $pdo->prepare("SELECT COUNT(*) c FROM clientes_web_pdvoff $whereSql");
  ...
  if ($action === 'get') {
    ...
    $stmt = $pdo->prepare("SELECT * FROM clientes_web_pdvoff WHERE id=? LIMIT 1");
    $stmt->execute([$id]);
    $row = $stmt->fetch();
  ...
  if ($action === 'create') {
    ...
    $stmt = $pdo->prepare("
      ...
      VALUES (?, ?, ?, ?, ?, ?, NOW(), NOW())
    ");
    $stmt->execute([$cod, $cli, $srv, $cx, $tipo, $obs]);
  ...
  if ($action === 'update') {
    ...
    $stmt = $pdo->prepare("
      ...
      LIMIT 1
    ");
    $stmt->execute([$cod, $cli, $srv, $cx, $obs, $id]);
  ...
  if ($action === 'delete') {
    ...
    $pdo->prepare("DELETE FROM clientes_web_pdvoff WHERE id=? LIMIT 1")
        ->execute([$id]);
