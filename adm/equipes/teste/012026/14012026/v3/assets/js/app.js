...
function loadModule(tab) {
  try {
    ...
    if (tab === 'dashboard' && typeof loadDashboard === 'function') {
      loadDashboard();
      if (typeof initDashboardCollapse === 'function') {
...
function showToast(message, type = 'info', timeout = 3000) {
  ...
  document.querySelectorAll('.dashboard-collapsible').forEach(card => {
    ...
    const isCollapsed = localStorage.getItem(`dash_collapse_${id}`) === '1';
    ...
    header.addEventListener('click', () => {
      ...
      localStorage.setItem(`dash_collapse_${id}`, col ? '1' : '0');
    });
  });
}
