...
try {
  ...
  if ($action === 'create') {
    ...
    $stmt = $pdo->prepare("
      ...
      (cod_cliente, cliente, acesso_server, tipo_acesso, status, observacao, criado_em, atualizado_em)
      VALUES (?, ?, ?, ?, ?, ?, NOW(), NOW())
    ");

    $stmt->execute([$cod, $cliente, $server, $tipo, $status, $obs]);
  ...
  if ($action === 'update') {
    ...
    $stmt->execute([$cod, $cliente, $server, $tipo, $status, $obs, $id]);
