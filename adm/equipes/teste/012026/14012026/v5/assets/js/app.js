...
function setPageInfo(tab) {
 ...
 relatorios:['Relatórios', 'Indicadores e estatísticas do sistema']
  };
...
function updateTimerUI(){
  if (!timerEl) return;
  timerEl.textContent = `⏳ Atualiza em ${formatTime(autoRefreshSeconds)}`;
}
...
/* Reinicia contador ao clicar manualmente */
btnRefresh?.addEventListener('click', () => {
  autoRefreshSeconds = AUTO_REFRESH_MINUTES * 60;
  updateTimerUI();
});
