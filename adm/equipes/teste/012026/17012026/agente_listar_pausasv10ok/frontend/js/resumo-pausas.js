 ...
 * - ALERTA VISUAL quando pausa estoura limite
 * ===================================================
 */
...
/* ===============================
   LIMITES DE PAUSA (MINUTOS)
=============================== */
const LIMITE_PAUSA = {
  1: 60, // Almoço → 60 min
  2: 30, // Lanche manhã → 30 min
  3: 30  // Lanche tarde → 30 min
};
...
/* ===============================
   PAUSA ESTOURADA?
=============================== */
function pausaEstourada(a) {
  if (!a.current_pause?.time_start || !a.current_pause?.reason?.id) return false;

  const limiteMin = LIMITE_PAUSA[a.current_pause.reason.id];
  if (!limiteMin) return false;

  const inicio = new Date(a.current_pause.time_start);
  const diffMin = (Date.now() - inicio) / 1000 / 60;
...
function renderPainel() {
  ...
  contador.innerHTML = `🟢 ${online} online • 🔴 ${offline} offline • ⏸ ${pausados} em pausa`;
  ...
  Object.entries(filas).forEach(([fila, lista]) => {
    ...
    lista.forEach(a => {
      ...
      if (status === "paused" && pausaEstourada(a)) {
