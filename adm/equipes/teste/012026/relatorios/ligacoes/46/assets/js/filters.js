...
function bindFilterEvents() {
  ...
  autoFields.forEach(id => {
    ...
    if (el) el.addEventListener('change', autoLoadCalls);
  });

  const phone = document.getElementById('phone_number');
  if (phone) phone.addEventListener('input', autoLoadCalls);

  const btnBuscar = document.getElementById('btnBuscar');
  if (btnBuscar) btnBuscar.addEventListener('click', () => loadCalls(1));
}
