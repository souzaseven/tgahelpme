...
let buscaOperador = '';    // texto digitado no campo de busca por nome
let houveAlteracaoNaoSalva = false; // true quando há edições ainda não enviadas ao "Salvar Semana"
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('busca-operador').addEventListener('input', (event) => {
        buscaOperador = event.target.value || '';
        aplicarFiltro();
    });
    ...
    if (btnNotificacao) {
        ...
        // "Última vista" fica salva por usuário (não por janela fixa de tempo),
        // assim nada que mudou enquanto o admin estava ausente passa despercebido.
        const chaveUltimaVista = `repasse:notif-vista:${USUARIO.id}`;
        const obterUltimaVista = () => localStorage.getItem(chaveUltimaVista) || '';
        const salvarUltimaVista = (ts) => { try { localStorage.setItem(chaveUltimaVista, ts); } catch (e) { /* ignora */ } };

        async function buscarNotificacoes(atualizarBadge = true) {
            try {
                const desde = obterUltimaVista();
                const url = 'api/notificacoes.php' + (desde ? `?desde=${encodeURIComponent(desde)}` : '');
        ...
        btnNotificacao.addEventListener('click', async () => {
            ...
            if (notificacoesNovas > 0) {
                ...
                badge.style.display = 'none';
                // notificacoesCache vem ordenado do mais novo pro mais antigo — usa
                // o timestamp do servidor (não o relógio do navegador) como marco.
                salvarUltimaVista(notificacoesCache[0].alterado_em);
            } else {
                let html = `<div class="notificacao-header">
                    <span>🔔 Novos tickets recebidos</span>
                    <button class="fechar" id="fechar-painel-notificacao">&times;</button>
                </div><div class="notificacao-vazio">Nenhum ticket novo desde a última vez que você conferiu.</div>`;
                mostrarPainelNotificacao(html);
            }
        });
    }
...
function aplicarFiltro() {
    const termoBusca = buscaOperador.trim().toLocaleLowerCase('pt-BR');

    // Filtro de linha (TEF / Troca de Regime / férias / busca por nome)
    document.querySelectorAll('.repasse-table tbody tr').forEach(tr => {
        ...
        if (visivel && termoBusca) {
            visivel = (tr.dataset.opNomeLc || '').includes(termoBusca);
        }
        tr.style.display = visivel ? '' : 'none';
    });

    // Filtro de equipe (mostra/oculta cards inteiros).
    // Com busca ativa, mostra qualquer equipe que tenha pelo menos um resultado,
    // ignorando o filtro de equipe selecionado (busca é sempre global).
    document.querySelectorAll('.team-card').forEach(card => {
        let visivel;
        if (termoBusca) {
            visivel = Array.from(card.querySelectorAll('tbody tr')).some(tr => tr.style.display !== 'none');
        } else {
            visivel = filtroEquipe === '' || card.dataset.lider === filtroEquipe;
        }
...
function navSemana(offset) {
    if (houveAlteracaoNaoSalva && !confirm('Você tem alterações não salvas nesta semana. Elas ficam guardadas neste navegador e serão restauradas quando você voltar aqui, mas só valem pra valer (e outros líderes só vão ver) depois de clicar em "Salvar Semana". Trocar de semana mesmo assim?')) {
        return;
    }
...
async function carregarDados() {
    ...
    try {
        ...
        // Nunca renderiza a tabela como se estivesse "vazia de verdade" quando a
        // busca falhou de verdade — isso já causou perda de dados uma vez (a tela
        // mostrava zero, e um "Salvar Semana" nesse estado sobrescrevia os valores
        // reais no banco). Erro aqui trava a tela com aviso, não deixa passar.
        if (!resOps.ok || !resRep.ok) {
            const erroResp = !resRep.ok ? await resRep.json().catch(() => null) : null;
            document.getElementById('conteudo').innerHTML = `
                <div class="erro-carregamento">
                    ⚠️ Não foi possível carregar os dados desta semana do servidor.<br>
                    <strong>Os valores abaixo NÃO são reais — não clique em "Salvar Semana" agora, isso apagaria dados.</strong><br>
                    ${erroResp?.erro ? `<small>${htmlEsc(erroResp.erro)}</small><br>` : ''}
                    <button onclick="carregarDados()" class="btn btn-outline" style="margin-top:12px">🔄 Tentar de novo</button>
                </div>`;
            document.getElementById('btn-salvar').disabled = true;
            mostrarToast('Falha ao carregar a semana — veja o aviso na tela.', 'error');
            return;
        }
        ...
        marcarComoSalvo(); // dados recém-carregados do banco: nada pendente ainda

        // Havia algo digitado nessa semana que nunca chegou a ser salvo
        // (ex.: fechou a aba, deu F5, ou a internet caiu no meio)? Restaura.
        const rascunho = carregarRascunhoLocal(semanaAtual);
        if (rascunho && aplicarRascunhoNaTela(rascunho.payload)) {
            marcarAlteracaoNaoSalva();
            aplicarFiltro();
            atualizarTodosBadgesEquipe();
            const quando = new Date(rascunho.salvoEm).toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' });
            mostrarToast(`⚠️ Restauramos edições de ${quando} que não tinham sido salvas. Revise e clique em "Salvar Semana".`, 'info', 6000);
        }
    } catch (err) {
        console.error(err);
...
function renderizarLinhas(lider, operadores) {
    ...
    operadores.forEach((op, idx) => {
        ...
        const dados = repasseData[op.id] || { seg:0, ter:0, qua:0, qui:0, sex:0, sab:0, em_ferias:false, nao_repassar:false, nao_repassar_herdado:false };
...
function criarLinha(lider, op, dados, ordem, feriasPeriodo = null) {
    ...
    const naoRepassarHerdado = !!dados.nao_repassar_herdado;
    if (emFerias) {
    ...
    tr.innerHTML = `
        ...
        <td class="nao-repassar-cell">
            <label class="nao-repassar-label" ${naoRepassarHerdado ? 'title="Herdado da semana anterior — ainda não confirmado nesta semana. Salve a semana para confirmar."' : ''}>
                <input type="checkbox" class="nao-repassar-check" ${naoRepassar ? 'checked' : ''}>
                <span class="nao-repassar-label-text">Não Repassar</span>
                ${naoRepassarHerdado ? '<span class="nao-repassar-herdado-tag">↩ herdado</span>' : ''}
            </label>
        </td>
    ...
    tbodyList.forEach(tbody => {
        ...
        if (ordemTabela === 'ordem_original') {
            rows.sort((a, b) => {
                ...
                // Usa Number.isNaN em vez de "||" para não tratar ordem 0 (primeiro
                // operador da equipe) como "sem posição" e jogá-lo para o final.
                const ordemA = parseInt(a.dataset.ordem, 10);
                const ordemB = parseInt(b.dataset.ordem, 10);
                return (Number.isNaN(ordemA) ? 9999 : ordemA) - (Number.isNaN(ordemB) ? 9999 : ordemB);
            });
        } else {
...
function atualizarRodape(lider) {
    ...
    const celsDias = dias.map(d => {
        const titulo = `Total de tickets repassados pela equipe na ${NOMES_DIAS[d]}`;
        return `<td title="${titulo}">${totDias[d]}</td>`;
    }).join('');
    tfoot.innerHTML = `
        <tr class="tfoot-totals">
            <td title="Soma de tickets repassados por dia, considerando toda a equipe">Total por dia</td><td></td><td></td>
            ${celsDias}
            <td title="Total de tickets repassados pela equipe na semana inteira">${totGeral}</td><td></td>
        </tr>
    `;
...
function marcarAlteracaoNaoSalva() {
    houveAlteracaoNaoSalva = true;
    const indicador = document.getElementById('indicador-nao-salvo');
...
window.addEventListener('beforeunload', (event) => {
    if (!houveAlteracaoNaoSalva) return;
    event.preventDefault();
    event.returnValue = '';
});

// ─── Rascunho local (sobrevive a F5/fechar aba sem salvar) ────────────────────
// Guarda no localStorage do navegador o que está digitado mas ainda não foi
// enviado ao "Salvar Semana", por semana. Ao reabrir a mesma semana (mesmo após
// atualizar a página ou fechar e voltar), o rascunho é restaurado por cima dos
// dados do banco. Só é apagado quando a semana é realmente salva no servidor.
function chaveRascunho(semana) {
    return `repasse:rascunho:${semana}`;
}
...
function salvarRascunhoLocal() {
    try {
        ...
        localStorage.setItem(chaveRascunho(semanaAtual), JSON.stringify({
            salvoEm: Date.now(),
            payload,
        }));
    } catch (e) {
        // localStorage indisponível/cheio — sem rascunho, mas não quebra a tela
    }
}

function removerRascunhoLocal(semana) {
    try { localStorage.removeItem(chaveRascunho(semana)); } catch (e) { /* ignora */ }
}

function carregarRascunhoLocal(semana) {
    try {
        const raw = localStorage.getItem(chaveRascunho(semana));
...
// Sobrepõe os valores de um rascunho salvo nas linhas já renderizadas.
// Retorna true se algum valor divergente foi de fato aplicado.
function aplicarRascunhoNaTela(payload) {
    if (!payload || !Array.isArray(payload.operadores)) return false;

    const dias = ['seg', 'ter', 'qua', 'qui', 'sex', 'sab'];
    let aplicouAlgo = false;

    payload.operadores.forEach(op => {
        const tr = document.querySelector(`.repasse-table tbody tr[data-op-id="${op.operador_id}"]`);
        if (!tr) return; // operador não está mais nessa semana (removido, virou férias, etc.)

        const lider = tr.dataset.lider;

        const check = tr.querySelector('.nao-repassar-check');
        if (check && check.checked !== !!op.nao_repassar) {
            check.checked = !!op.nao_repassar;
            toggleNaoRepassar(tr, check.checked, lider);
            aplicouAlgo = true;
        }

        dias.forEach(d => {
            const inp = tr.querySelector(`.dia-input[data-dia="${d}"]`);
            if (!inp || inp.disabled) return; // bloqueado por férias/não-repassar: rascunho não se aplica
            const valorRascunho = Math.max(0, parseInt(op[d]) || 0);
            if ((parseInt(inp.value) || 0) !== valorRascunho) {
                inp.value = valorRascunho;
                aplicouAlgo = true;
            }
        });
...
async function salvarSemana() {
    ...
    try {
        ...
        if (json.success) {
            removerRascunhoLocal(semanaAtual);
            await carregarDados();
...
function atualizarTituloSemana() {
    ...
    const segundaHoje = normalizarParaSegunda(obterSegundaAtual());
    let selo = '';
    if (semanaAtual > segundaHoje) {
        selo = '<span class="semana-selo semana-selo-futura" title="Essa semana ainda não começou — os valores em 0 são apenas o padrão, não significam que deu zero tickets.">🔮 Semana futura</span>';
    } else if (semanaAtual === segundaHoje) {
        selo = '<span class="semana-selo semana-selo-atual">📍 Semana atual</span>';
    } else {
        selo = '<span class="semana-selo semana-selo-passada" title="Essa semana já encerrou — os valores aqui são o que de fato foi repassado.">🗂 Semana passada</span>';
    }

    document.getElementById('semana-titulo').innerHTML = `Semana de ${inicio} a ${fim} ${selo}`;
