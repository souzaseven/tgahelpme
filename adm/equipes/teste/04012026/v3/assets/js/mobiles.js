...
async function loadMobiles() {
  // Garante que a view já existe (e que a tabela está no DOM)
  const view = document.getElementById('view-mobiles');
  if (!view) return;

  // Se a tabela não existir (caso você mude o HTML depois), não dá erro.
...
function bindMobilesEventsOnce() {
  ...
  document.addEventListener('click', (e) => {
    ...
    if (t && t.id === 'btnNovoMobile') {
      novoMobile();
      return;
    }

    if (t && t.id === 'mobilePrev') {
...
function renderMobile(rows) {
  ...
  rows.forEach(r => {
    ...
    // ações por dataset (sem onclick inline)
    tr.querySelector('[data-action="edit"]').addEventListener('click', () => editarMobile(r.id));
    tr.querySelector('[data-action="del"]').addEventListener('click', () => excluirMobile(r.id));
...
/* ==========================
   EXPOR FUNÇÕES (caso ainda exista onclick em algum lugar)
========================== */
