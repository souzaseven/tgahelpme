...
if ($metodo === 'POST') {
    ...
    if (($d['acao'] ?? '') === 'alterar_status_bulk') {
        ...
        $ph = implode(',', array_fill(0, count($ids), '?'));
        $pdo->prepare("UPDATE `{$p}transacoes` SET status=?, atualizado_em=NOW() WHERE id IN ($ph) AND tipo='despesa'")
            ->execute([$status, ...$ids]);
    ...
    // Calcula mês/ano da fatura (mês de PAGAMENTO = padrão brasileiro)
    // Compra dia 3, fechamento dia 10 → fecha em junho → paga em julho → retorna [7, ano]
    $calcMesFatura = static function(string $dataCompra, int $diaFech): array {
        [$a, $m, $d2] = array_map('intval', explode('-', $dataCompra));
        // Mês de fechamento
        $mClose = $d2 <= $diaFech ? (int)$m : ($m === 12 ? 1 : $m + 1);
        $aClose = $d2 <= $diaFech ? (int)$a : ($m === 12 ? $a + 1 : $a);
        ...
        $stmt = $pdo->prepare(
            "UPDATE `{$p}transacoes`
             SET descricao=?, valor=?, data=?, categoria_id=?, conta_id=?,
                 cartao_id=?, responsavel_id=?, terceiro_id=?, status=?, observacao=?, tags=?, data_vencimento=?, atualizado_em=NOW()
             WHERE id=? AND tipo='despesa'"
        );
        $stmt->execute([$descricao, $valor, $data, $catId, $contaId, $cartaoId, $respId, $terceiroId, $status, $obs, $tags, $dataVenc, $id]);
    ...
    try {
        if ($isDivisao) {
            ...
            $n      = count($divisao);
            $vlUnit = round($valor / $n, 2);
            // Ajuste de centavos para fechar exato
            $vlResto = round($valor - ($vlUnit * $n), 2);
            ...
            $stmtDiv = $pdo->prepare(
                "INSERT INTO `{$p}transacoes`
                 (tipo, descricao, valor, data, categoria_id, conta_id, cartao_id,
                  responsavel_id, status, observacao, tags, grupo_id, fatura_id, data_vencimento)
                 VALUES ('despesa',?,?,?,?,?,?,?,?,?,?,?,?,?)"
            );

            $grupoId  = null;
            $obsDiv   = $obs ? $obs . " [divisão 1/$n]" : "Divisão 1/$n";

            foreach ($divisao as $idx => $rIdDiv) {
                $rIdDiv  = ((int)$rIdDiv) ?: null;
                $vlAtual = ($idx === 0) ? $vlUnit + $vlResto : $vlUnit;
                $obsAtual = $obs ? $obs . " [divisão " . ($idx+1) . "/$n]" : "Divisão " . ($idx+1) . "/$n";

                $stmtDiv->execute([
                    $descricao, $vlAtual, $data, $catId, $contaId, $cartaoId,
                    $rIdDiv, $status, $obsAtual, $tags, $grupoId, $fatIdDiv, $dvDiv,
                ]);
        ...
        $sql  = "INSERT INTO `{$p}transacoes`
                 (tipo, descricao, valor, data, categoria_id, conta_id, cartao_id,
                  responsavel_id, terceiro_id, status, observacao, tags, parcela_atual, parcela_total, fatura_id, data_vencimento)
                 VALUES ('despesa',?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
