...
if ($acao === 'previsao') {
    ...
    foreach ($recs as $rec) {
        // Verifica se deve gerar no mês (replica deveGerarNoMes)
        $dtIni = new DateTime(substr($rec['data_inicio'], 0, 7) . '-01');
        if ($dtAlvo < $dtIni) continue;
        if (!empty($rec['data_fim'])) {
            $dtFim = new DateTime(substr($rec['data_fim'], 0, 7) . '-01');
            if ($dtAlvo > $dtFim) continue;
        }
        $diff = ($dtAlvo->format('Y') - $dtIni->format('Y')) * 12
              + ((int)$dtAlvo->format('m') - (int)$dtIni->format('m'));
        $per  = ['diaria'=>0,'semanal'=>0,'quinzenal'=>0,'mensal'=>1,'bimestral'=>2,
                 'trimestral'=>3,'semestral'=>6,'anual'=>12][$rec['frequencia']] ?? 1;
        if (!($per === 0 || $diff % $per === 0)) continue;

        // Busca transação gerada para este mês
        $stmtTx->execute([$rec['id'], $mes, $ano]);
        $tx = $stmtTx->fetch();
