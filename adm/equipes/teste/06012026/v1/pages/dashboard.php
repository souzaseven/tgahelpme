...
$dtProx   = new DateTime(sprintf('%04d-%02d-01', $anoProx, $mesProx));
$recsAtivas = $pdo->query("SELECT * FROM `{$p}recorrencias` WHERE ativo=1")->fetchAll();
$projRec = 0.0; $projDes = 0.0;
foreach ($recsAtivas as $r) {
    $dtIni = new DateTime(substr($r['data_inicio'], 0, 7) . '-01');
    if ($dtProx < $dtIni) continue;
    if (!empty($r['data_fim'])) {
        $dtFim = new DateTime(substr($r['data_fim'], 0, 7) . '-01');
        if ($dtProx > $dtFim) continue;
    }
    $diff = ($dtProx->format('Y') - $dtIni->format('Y')) * 12
          + ((int)$dtProx->format('m') - (int)$dtIni->format('m'));
    $per  = ['diaria'=>0,'semanal'=>0,'quinzenal'=>0,'mensal'=>1,'bimestral'=>2,
             'trimestral'=>3,'semestral'=>6,'anual'=>12][$r['frequencia']] ?? 1;
    if (!($per === 0 || $diff % $per === 0)) continue;
...
// Fluxo de caixa 6 meses (para gráfico de linha)
$fluxo = [];
for ($i = 5; $i >= 0; $i--) {
    ...
    $isCurMes    = ($mRef === $mes && $yRef === $ano);
    $filtroDes   = $isCurMes ? "status != 'cancelado'" : "status = 'pago'";

    $sR = $pdo->prepare("SELECT COALESCE(SUM(valor),0) FROM `{$p}transacoes` WHERE tipo='receita' AND status='pago' AND MONTH(data)=? AND YEAR(data)=?$_rWhere");
    $sR->execute(array_merge([$mRef, $yRef], $_rParams));
    $sD = $pdo->prepare("SELECT COALESCE(SUM(valor),0) FROM `{$p}transacoes` WHERE tipo='despesa' AND $filtroDes AND MONTH(data)=? AND YEAR(data)=?$_rWhere");
    $sD->execute(array_merge([$mRef, $yRef], $_rParams));

    $fluxo[] = ['label' => $label, 'rec' => (float)$sR->fetchColumn(), 'des' => (float)$sD->fetchColumn()];
}
