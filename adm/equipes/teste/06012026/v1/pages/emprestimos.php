...
function brlE(float $v): string { return 'R$ '.number_format($v,2,',','.'); }
function pctE(float $v): string { return number_format($v,2,',','.').'%'; }
...
.modal-box { background:var(--bg-800); border:1px solid var(--border); border-radius:var(--radius-lg); width:100%; max-width:600px; max-height:90vh; display:flex; flex-direction:column; box-shadow:var(--shadow-lg); animation:modalIn .18s ease; }
...
<div class="kpi-grid" style="grid-template-columns:repeat(4,1fr);margin-bottom:1.25rem">
    <div class="kpi-card rose">
        ...
        <div class="kpi-value"><?= brlE($totSaldo) ?></div>
    ...
    <div class="kpi-card amber">
        ...
        <div class="kpi-value"><?= brlE($totParcela) ?></div>
    ...
    <div class="kpi-card indigo">
        <div class="kpi-header">
            ...
            <div class="kpi-icon indigo"><i class="fa-solid fa-file-invoice-dollar"></i></div>
        </div>
...
<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(300px,1fr));gap:1rem">
    ...
    <div class="emp-card <?= htmlspecialchars($e['status']) ?>"
        ...
        <div class="emp-saldo"><?= brlE((float)$e['saldo_devedor']) ?></div>
        <div class="emp-parc">
            Parcela: <?= brlE((float)$e['valor_parcela']) ?> &nbsp;·&nbsp;
            <?= $e['taxa_juros_mensal'] > 0 ? pctE((float)$e['taxa_juros_mensal']).' a.m.' : 'Sem juros' ?>
        </div>
        ...
        <div class="d-flex justify-between text-xs text-muted" style="margin-top:.5rem">
            ...
            <span class="fw-600"><?= brlE((float)$e['proxima_valor']) ?></span>
        </div>
        <?php endif ?>
