...
function brlImp(v){ return 'R$ ' + parseFloat(v||0).toFixed(2).replace('.',',').replace(/\B(?=(\d{3})+(?!\d))/g,'.'); }

async function processarArquivo(file) {
    if (!file) return;
    if (!document.getElementById('impConta').value) {
        alert('Selecione a conta destino antes de importar.');
        return;
    }
    ...
    } catch(e) { alert('Erro ao processar arquivo: ' + e.message); }
}

function renderPreview(lista) {
    const tbody = document.getElementById('impTabelaBody');
    if (!lista.length) { alert('Nenhuma transação encontrada no arquivo.'); return; }
...
async function confirmarImportacao() {
    ...
    if (!contaId) { alert('Conta não selecionada.'); return; }
    ...
    try {
        ...
        if (typeof toast === 'function') toast(`${json.importadas} transações importadas com sucesso!`, 'success');
        else alert(`${json.importadas} transações importadas!`);
        resetarImportacao();
    } catch(e) {
