...
<div class="kpi-grid" style="margin-bottom:1.25rem">
    ...
    <div class="kpi-card violet">
        <div class="kpi-header">
            ...
            <div class="kpi-icon violet"><i class="fa-solid fa-receipt"></i></div>
        </div>
...
function renderTabela(dados, nomeMes, ano) {
    ...
    tbody.innerHTML = paginados.map(tx => {
        ...
        const btnRecebido = tx.status !== 'pago'
            ? `<button class="btn-icon" onclick="marcarRecebido(${+tx.id})" title="Marcar como recebido"
        ...
        return `<tr>
            ...
            <td class="text-right fw-600 text-emerald">+${brl(tx.valor)}</td>
            <td>
                <div class="d-flex gap-1" style="justify-content:flex-end">
                    ...
                    <button class="btn-icon" onclick="excluirReceita(${+tx.id})" title="Excluir"
...
function abrirModalBulkStatusRec() {
    if (!_selecionados.size) return;
    document.getElementById('modalBulkStatusRec').style.display = 'flex';
}

function fecharModalBulkStatusRec() {
    document.getElementById('modalBulkStatusRec').style.display = 'none';
}

async function confirmarAlterarStatusRec() {
    const novoStatus = document.getElementById('bulkNovoStatusRec').value;
    fecharModalBulkStatusRec();
    try {
        const res  = await fetch('backend/api/receitas.php', {
            method: 'POST',
            headers: {'Content-Type': 'application/json'},
...
function renderTemplates(lista) {
    ...
    if (!lista.length) {
        wrap.innerHTML = `<div style="text-align:center;padding:2rem;color:var(--text-600)">
...
<!-- ── Bulk action bar ───────────────────────────────────── -->
<div id="bulkBarRec" style="display:none;position:fixed;bottom:0;left:0;right:0;z-index:400;
    ...
    <span id="bulkCountRec" style="font-size:.85rem;font-weight:700;color:var(--text-100)">0 selecionados</span>
    <div style="flex:1"></div>
    <button onclick="abrirModalBulkStatusRec()"
...
<div id="modalBulkStatusRec" onclick="if(event.target===this)fecharModalBulkStatusRec()"
    ...
    <div style="background:var(--bg-800);border:1px solid var(--border);border-radius:var(--radius-lg);
        ...
        <select id="bulkNovoStatusRec" class="form-control" style="margin-bottom:1.25rem">
            <option value="pago">Recebido</option>
            <option value="pendente">Pendente</option>
            <option value="cancelado">Cancelado</option>
        </select>
        <div style="display:flex;gap:.5rem;justify-content:flex-end">
            <button onclick="fecharModalBulkStatusRec()"
            ...
            <button onclick="confirmarAlterarStatusRec()"
                    style="padding:.35rem .85rem;font-size:.82rem;font-weight:600;border-radius:var(--radius);
