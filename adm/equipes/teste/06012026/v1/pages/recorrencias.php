...
.modal-box { background:var(--bg-800); border:1px solid var(--border); border-radius:var(--radius-lg); width:100%; max-width:580px; max-height:90vh; display:flex; flex-direction:column; box-shadow:var(--shadow-lg); animation:modalIn .18s ease; }
...
/* ── Filtros ──────────────────────────────────────────────── */
.filters-bar { background:var(--bg-800); border:1px solid var(--border); border-radius:var(--radius-lg); padding:.875rem 1.25rem; margin-bottom:1.25rem; display:flex; flex-wrap:wrap; gap:.75rem; align-items:flex-end; }
.filter-group { display:flex; flex-direction:column; gap:.35rem; }
.filter-label { font-size:.7rem; font-weight:600; color:var(--text-600); text-transform:uppercase; letter-spacing:.05em; }
...
<div class="kpi-grid" style="grid-template-columns:repeat(3,1fr);margin-bottom:1.25rem">
    <div class="kpi-card rose">
        ...
        <div class="kpi-value"><?= brlR($totalFixoDes) ?></div>
    ...
    <div class="kpi-card emerald">
        ...
        <div class="kpi-value"><?= brlR($totalFixoRec) ?></div>
        <div class="kpi-trend neutral">
            <i class="fa-solid fa-circle-info fa-xs"></i> Equivalente mensal
        </div>
    </div>
    <div class="kpi-card <?= ($totalFixoRec - $totalFixoDes) >= 0 ? 'indigo' : 'amber' ?>">
        ...
        <div class="kpi-value"><?= brlR(abs($totalFixoRec - $totalFixoDes)) ?></div>
        ...
        <table>
            ...
            <tr class="<?= !$r['ativo'] ? 'inativa' : '' ?>" data-id="<?= $r['id'] ?>">
                ...
                <td class="text-right fw-600 <?= $r['tipo']==='receita' ? 'text-emerald' : 'text-rose' ?>">
                    <?= $r['tipo']==='receita' ? '+' : '-' ?><?= brlR((float)$r['valor']) ?>
                </td>
                <td>
