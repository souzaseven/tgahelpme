...
/* ============================================================
   GET — LISTAGEM + FILTROS + PAGINAÇÃO (ESTÁVEL)
============================================================ */
if ($method === "GET") {

    // 🔒 Proteção defensiva (evita erro 500 no LIKE)
    $q        = clean_str($_GET["q"] ?? "");
    $q        = substr($q, 0, 100);
    ...
    // 🔥 LIMIT/OFFSET SEM BIND (CORREÇÃO DEFINITIVA)
    $limit  = (int)$perPage;
    $offset = (int)$offset;
...
/* ============================================================
   POST — CRIAR CLIENTE (COM REGRA MIGRADO)
============================================================ */
if ($method === "POST") {
    ...
    if ($info_adicional === "MIGRADO" && $firebird !== "5.0") {
        respostaJSON([
            "success" => false,
            "error"   => "Migrado só é permitido para Firebird 5.0."
        ], 422);
    }
...
/* ============================================================
   PUT / DELETE
   (mantidos como já estavam funcionando)
============================================================ */
