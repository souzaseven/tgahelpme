...
/* ==========================
   TOOLTIP DO MENU COLAPSADO
   (a sidebar tem overflow-y:auto, então um tooltip
   posicionado via CSS puro seria cortado — por isso
   usamos um único elemento fixo, fora da sidebar)
========================== */
(() => {
  ...
  sidebar.querySelectorAll('.menu-item').forEach(item => {
    item.addEventListener('mouseenter', () => {
      ...
      const texto = item.querySelector('.mi-label')?.textContent.trim();
      if (!texto) return;
...
function openModal({ title, entity, id = '', fields = [] }) {
  ...
  modalFields.className  = 'form-grid'; // limpa modificadores (ex.: sidebar de integrações) de aberturas anteriores
