  ...
  let html = `
    <div class="srv-card-top">
      ...
      <div class="srv-title-block">
        <h3 class="cell-with-copy">
          ${esc(r.icone)} ${esc(r.titulo)}
          ${r.titulo ? `<button class="btn-copy" data-copy="${esc(r.titulo)}" title="Copiar"><i class="fas fa-copy"></i></button>` : ''}
        </h3>
        ${r.descricao ? `<p class="muted cell-with-copy">${esc(r.descricao)} <button class="btn-copy" data-copy="${esc(r.descricao)}" title="Copiar"><i class="fas fa-copy"></i></button></p>` : ''}
      </div>
    ...
    itens.forEach(item => {
      html += `<li>
        <span class="cell-with-copy">
          ...
          ${item.nome ? `<button class="btn-copy" data-copy="${esc(item.nome)}" title="Copiar"><i class="fas fa-copy"></i></button>` : ''}
        </span>
        ${item.tag  ? `<span class="tag">${esc(item.tag)}</span>` : ''}
        ${item.desc ? `<span class="cell-with-copy"><small>${esc(item.desc)}</small><button class="btn-copy" data-copy="${esc(item.desc)}" title="Copiar"><i class="fas fa-copy"></i></button></span>` : ''}
      </li>`;
    });
    html += '</ul>';
  }
