  ...
  Semana de Telefone <span style="opacity:.6">– Matriz</span>
    ...
    <div class="rotacao-telefone-grid-resumo">
      ...
      <div class="rotacao-telefone-item-resumo proxima">
        ...
        <?php
          // Calcular "Equipe Seguinte" (após a próxima)
          $idxAtual = array_search($rotacao['atual'], $ordem);
          $idxSeguinte = ($idxAtual + 2) % count($ordem);
          $equipeSeguinte = $ordem[$idxSeguinte];
        ?>
        <div class="rotacao-telefone-item-resumo seguinte" style="background: linear-gradient(135deg, #312e81, #6366f1); box-shadow: 0 0 0 1px rgba(99,102,241,.35), 0 10px 25px rgba(49,46,129,.25);">
          <span class="rotulo">Equipe Seguinte</span>
          <span class="valor"><?= htmlspecialchars($equipeSeguinte) ?></span>
          <div class="fila">Próxima previsão</div>
        </div>
...
.header-actions {
  ...
  <!-- Card compacto de resumo das semanas -->
  <div id="resumo-semanas-mini" style="max-width:520px; margin:32px auto 0 auto; background:rgba(30,41,59,.85); border-radius:14px; box-shadow:0 2px 16px rgba(0,0,0,.10); padding:18px 12px 10px 12px;">
    ...
    <table style="width:100%; font-size:13px; border-collapse:collapse;">
      <thead>
        <tr style="background:rgba(51,65,85,.95); color:#a5b4fc;">
          ...
          <th style="padding:6px 4px;">Tel</th>
          <th style="padding:6px 4px;">Chat</th>
          <th style="padding:6px 4px; border-radius:0 8px 8px 0;">Comp</th>
        </tr>
      </thead>
      <tbody>
        <?php foreach ($resumoSemanas as $sem): ?>
