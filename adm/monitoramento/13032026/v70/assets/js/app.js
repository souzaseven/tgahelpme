...
let countdownAgentes  = 3;
let countdownLigacoes = 10;
...
let pausasDisponiveis = [];

async function carregarPausasDisponiveis() {
    try {
        const res  = await fetch('api/pausas_disponiveis.php');
        const json = await res.json();
        pausasDisponiveis = json.data || [];
    } catch(e) {
        console.error("Erro pausas disponíveis:", e);
    }
}
...
async function acaoAgente(agentId, acao, pauseId = null) {
    const body = new FormData();
    body.append("agent_id", agentId);
    body.append("acao", acao);
    if (pauseId) body.append("pause_id", pauseId);

    try {
        const res  = await fetch('api/acao_agente.php', { method: 'POST', body });
        const json = await res.json();
        if (json.meta?.status === 200) {
            mostrarToast("Ação executada", "online");
            await atualizarAgentes();
        } else {
            mostrarToast("Erro na ação", "offline");
        }
    } catch(e) {
        console.error("Erro ação agente:", e);
        mostrarToast("Erro na ação", "offline");
    }
}
...
function abrirMenuAcoes(btn, agentId, pausado) {
    const modal    = document.getElementById("modal-acoes");
    ...
    if (pausado) {
        ...
        item.className = "acao-item acao-despausar";
        item.textContent = "▶ Despausar";
        item.onclick = () => { fecharModalAcoes(); acaoAgente(agentId, "unpause"); };
        conteudo.appendChild(item);
    } else {
        const titulo = document.createElement("div");
        titulo.className = "acao-titulo";
        titulo.textContent = "Pausar como:";
        conteudo.appendChild(titulo);

   const pausasOcultas = ["Lanche manhã 15min", "Lanche tarde 15min"];

pausasDisponiveis
    .filter(p => !pausasOcultas.includes(p.description))
    .forEach(p => {
        const item = document.createElement("button");
        item.className = "acao-item acao-pausar";
        item.innerHTML = `<span class="acao-icon">⏸</span>${p.description}`;
        item.onclick = () => { fecharModalAcoes(); acaoAgente(agentId, "pause", p.id); };
        conteudo.appendChild(item);
    });
    }
    ...
    deslogar.className = "acao-item acao-deslogar";
    deslogar.textContent = "⏻ Deslogar";
    deslogar.onclick = () => {
        if (!confirm("Deslogar este agente?")) return;
        fecharModalAcoes();
        acaoAgente(agentId, "logoff");
    };
    conteudo.appendChild(deslogar);
...
async function carregarInicial() {
    mostrarLoading();
    try {
        await Promise.all([ carregarAgentes(), carregarLigacoes(), carregarPerformance(), carregarPausas(), carregarPausasDisponiveis() ]);
        montarFiltroFilas();
        sincronizarSelects();
        aplicarFiltros();
    } catch (error) {
...
/* Alerta de pausa longa */
const pausaInicio    = agent.current_pause?.time_start ? new Date(agent.current_pause.time_start) : null;
const pausaLimite    = agent.current_pause?.max_duration || null;
const pausaDecorrida = pausaInicio ? Math.floor((Date.now() - pausaInicio.getTime()) / 1000) : 0;
const pausaEstourou  = pausado && pausaLimite && pausaDecorrida > pausaLimite;

/* Horário de entrada */
const loginHoje = agent.last_login?.time_login
    ? new Date(agent.last_login.time_login).toLocaleTimeString('pt-BR', {
...
if (!online)        { statusLabel = "Offline";      statusClass = "badge-offline";  }
    ...
    statusLabel = (motivoPausa || "Pausado") + (pausaEstourou ? " ⚠️" : "");
    statusClass = pausaEstourou ? "badge-paused badge-pausa-alerta" : "badge-paused";
}
...
if (pausaEstourou) tr.classList.add("row-pausa-estourou");
...
${loginHoje ? `<span class="agent-ramal"> <br>Login ás: ${loginHoje}</span>` : ""}
    ...
    <div class="status-cell">
        ...
        <div class="acoes-wrap">
          <button class="acoes-btn" data-agent-id="${agent.id}" data-pausado="${pausado}">⚙ Mudar status</button>
            <div class="acoes-dropdown"></div>
        </div>` : ""}
    </div>
</td>
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    document.getElementById("multi-queue-trigger")?.addEventListener("click", function (e) {
        e.stopPropagation();
...
function mostrarToast(nome, tipo) {
    ...
    Notificacoes.adicionar(nome, tipo); 
