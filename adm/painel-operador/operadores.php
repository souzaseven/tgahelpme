...
$rotacao = ['Daniel Feix', 'Willian Pereira Reis', 'Alex Sandro Braulio']; // exemplo 
$atual = $telefoneLideres[0] ?? 'Não definido';
$index = array_search($atual, $rotacao);
$anterior = $rotacao[($index - 1 + count($rotacao)) % count($rotacao)];
$proxima  = $rotacao[($index + 1) % count($rotacao)];

$semana = [
    'telefone' => $atual,
    'telefoneAnterior' => $anterior,
    'telefoneProxima'  => $proxima,
    'chat' => $chatLideres[0] ?? 'Não definido'
];
