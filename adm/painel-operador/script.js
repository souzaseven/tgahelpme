...
if (data.semana) {
  document.getElementById('telefoneSemana').textContent = data.semana.telefone || 'Não definido';
  document.getElementById('telefoneAnterior').textContent = data.semana.telefoneAnterior || 'Indefinido';
  document.getElementById('telefoneAtual').textContent = data.semana.telefone || 'Indefinido';
  document.getElementById('telefoneProxima').textContent = data.semana.telefoneProxima || 'Indefinido';
}
    ...
    try {
        ...
        if (data.success) {
            ...
            allOperators = data.data.map(op => ({
                name: op.nome,
                monitor: op.lider,
                queues: processQueues(op.fila),
                link: op.link || '#'
            }));

            // Atualiza cards da semana
            updateWeeklyTeams(data.semana);
...
function processQueues(fila) {
    try {
        return typeof fila === 'string' ? JSON.parse(fila) : fila;
    } catch {
        return [fila].filter(Boolean);
    }
}

function updateWeeklyTeams(semana) {
    const setText = (id, text) => {
        const el = document.getElementById(id);
        if (el) el.textContent = text || 'Não definido';
    };

    setText('telefoneSemana', semana?.telefone);
    setText('chatSemana', semana?.chat);
}

function processQueues(fila) {
    try {
        const parsed = typeof fila === 'string' ? JSON.parse(fila) : fila;
        const queues = Array.isArray(parsed) ? parsed : [parsed];
        return queues.filter(q => q && String(q).trim());
    } catch (e) {
        return [fila || 'Sem fila definida'].filter(Boolean);
    }
}

function updateWeeklyTeams(semana) {
    const telefoneEl = document.getElementById('telefoneSemana');
    const chatEl = document.getElementById('chatSemana');
    
    if (semana && typeof semana === 'object') {
        telefoneEl.textContent = semana.telefone || 'Não definido';
        chatEl.textContent = semana.chat || 'Não definido';
    } else {
        telefoneEl.textContent = 'Dados inválidos';
        chatEl.textContent = 'Dados inválidos';
    }
}
...
function processOperator(op) {
    ...
    try {
        const filas = typeof op.fila === 'string' ? JSON.parse(op.fila) : op.fila;
        queues = Array.isArray(filas) ? filas : [filas];
    } catch (e) {
        queues = [op.fila || 'Sem fila'];
    }
    
    return {
        name: op.nome || 'Sem nome',
        monitor: op.lider || 'Sem equipe',
        queues: queues.filter(q => q && q.toString().trim()),
        link: op.link || '#'
    };
}
