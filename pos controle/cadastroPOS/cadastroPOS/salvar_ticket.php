...
try {
    ...
    if (empty($data['ticket']) || empty($data['empresa']) || empty($data['solicitacao']) || 
        empty($data['adquirente']) || empty($data['modelo']) || empty($data['status'])) {
        echo json_encode(['success' => false, 'error' => 'Campos obrigatórios não preenchidos']);
        exit;
    }

    $stmt = $pdo->prepare("INSERT INTO ticketsmarts 
        (ticket, empresa, cnpj, cod_cliente_tga, quant_smart, adquirente, modelo, solicitacao, anotacao, monitor, status, data_criacao, data_atualizacao) 
        VALUES 
        (:ticket, :empresa, :cnpj, :cod_cliente_tga, :quant_smart, :adquirente, :modelo, :solicitacao, :anotacao, :monitor, :status, NOW(), NOW())");
