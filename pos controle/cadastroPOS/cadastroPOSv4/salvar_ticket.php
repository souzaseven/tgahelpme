...
session_start(); // Precisa estar antes de acessar $_SESSION
$criado_por = $_SESSION['usuario'] ?? 'desconhecido';
...
try {
    ...
    $stmt = $pdo->prepare("INSERT INTO ticketsmarts 
        (ticket, empresa, cnpj, cod_cliente_tga, quant_smart, adquirente, modelo, solicitacao, anotacao, monitor, status, imagem, data_criacao, data_atualizacao, criado_por) 
        VALUES 
        (:ticket, :empresa, :cnpj, :cod_cliente_tga, :quant_smart, :adquirente, :modelo, :solicitacao, :anotacao, :monitor, :status, :imagem, NOW(), NOW(), :criado_por)");
